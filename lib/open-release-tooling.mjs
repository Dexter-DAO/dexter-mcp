...
export async function reviewedGitRemoteRefs({
  ...
  try {
    ...
    return textOutput(await runCommand('git', reviewedBareGitArgs(gitDir, [
      // Dependency provenance needs the commit behind annotated release tags;
      // exact-ref receipt callers retain the default ref-only contract.
      'ls-remote', ...(includePeeledTags ? [] : ['--refs']), remote, 'refs/*',
    ]), {
      cwd: controlRoot,
      encoding: 'utf8',
      env: toolEnvironment,
