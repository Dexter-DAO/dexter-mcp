import { parseIndexterRequestInputV2, projectIndexterRequestInputV2, classifyIndexterBodySchema } from './indexter-request-input-v2.mjs';
...
function requestFieldType(value) {
  ...
  return typeof type === 'string' && (REQUEST_INPUT_FIELD_TYPES.has(type)
    || (type === 'array' && requestArrayShape(value)))
    ? type
    : null;
}

function requestArrayShape(value) {
  const schema = value?.schema ?? value;
  if (schema?.type !== 'array') return null;
  if (!schema.items || typeof schema.items !== 'object' || Array.isArray(schema.items)
    || !REQUEST_INPUT_FIELD_TYPES.has(schema.items.type)) return null;
  const unsupported = ['$ref', '$dynamicRef', 'oneOf', 'anyOf', 'allOf', 'not', 'if', 'then', 'else',
    'prefixItems', 'contains', 'unevaluatedItems', 'uniqueItems', 'enum', 'const',
    'minimum', 'maximum', 'exclusiveMinimum', 'exclusiveMaximum', 'multipleOf',
    'minLength', 'maxLength', 'pattern', 'format'];
  if ([schema, schema.items].some((part) => unsupported.some((key) => Object.hasOwn(part, key)))) return null;
  if (['minItems', 'maxItems'].some((key) => Object.hasOwn(schema, key)
    && (!Number.isInteger(schema[key]) || schema[key] < 0))) return null;
  const minItems = schema.minItems ?? 0;
  const maxItems = Math.min(schema.maxItems ?? 32, 32);
  if (!Number.isInteger(minItems) || minItems < 0
    || !Number.isInteger(schema.maxItems ?? 32) || maxItems < minItems) return null;
  return { items: { type: schema.items.type }, minItems, maxItems };
}

function addRequestField(state, { name, location, type, required, items, minItems, maxItems }) {
  const safeName = safeRequestFieldName(name);
  if (
    !safeName
    || !REQUEST_INPUT_LOCATIONS.has(location)
    || !(REQUEST_INPUT_FIELD_TYPES.has(type) || (type === 'array' && location === 'body'
      && items && Object.keys(items).join(',') === 'type' && REQUEST_INPUT_FIELD_TYPES.has(items.type)
      && Number.isInteger(minItems) && minItems >= 0 && Number.isInteger(maxItems)
      && maxItems >= minItems && maxItems <= 32))
  ...
  state.fields.push({ name: safeName, location, type, required,
    ...(type === 'array' ? { items: { type: items.type }, minItems, maxItems } : {}) });
  return true;
}
  ...
  if (Array.isArray(value)) {
    ...
    for (const parameter of value) {
      ...
      if (!type || !addRequestField(state, {
        ...
        location,
        type,
        ...(type === 'array' ? requestArrayShape(parameter) : {}),
  ...
  if (hasWrapper) {
    for (const [name, definition] of entries) {
      ...
      if (!type || !addRequestField(state, {
        ...
        location,
        type,
        ...(type === 'array' ? requestArrayShape(definition) : {}),
...
function trustedRequestInput(value) {
  if (value?.version === 2) return parseIndexterRequestInputV2(value);
  if (
    !value
    || typeof value !== 'object'
    || Array.isArray(value)
  ...
  for (const field of value.fields) {
    if (
      ...
      || typeof field !== 'object'
      || Array.isArray(field)
      || Object.keys(field).sort().join(',') !== (field.type === 'array'
...
function projectRequestInput(source, method, requiresExplicitInput) {
  if (source.access?.kind === 'managed_resolvable') {
    const structure = classifyIndexterBodySchema(source.inputSchema);
    if (structure === 'unavailable') return null;
    if (structure === 'nested') {
      if (method === 'GET' || hasPublishedInput(source.pathParams)) return null;
      return projectIndexterRequestInputV2(source.inputSchema);
    }
  }
...
function endpointReview(safety) {
  return {
    inputReviewRequired: safety.requiresExplicitInput || safety.publishedInputPresent,
    authorityReviewRequired: safety.checkMayAffectProvider,
    authorizationBasis: 'current_task_and_active_permissions',
    nextAction: safety.checkMayAffectProvider
      ? 'verify_consequence_coverage_then_check'
      : 'use_known_inputs_and_check',
  };
}
...
function trustedProjectedEndpointAction(value, method, resourceId, resourceUrl, requestInput) {
  ...
  if (value.kind === 'endpoint_unavailable') {
    ...
    return {
      kind: 'endpoint_unavailable',
      label: value.reason === 'input_contract_unavailable'
  ...
  if (!requestInput || (requestInput.version === 2 && (method === 'GET' || resourceUrl !== null))) return null;
  ...
  return {
    ...
    resourceId,
    resourceUrl,
    review: endpointReview({ ...source, checkMayAffectProvider, publishedInputPresent }),
    safety: {
      requiresRequestReview,
      checkMayAffectProvider,
...
function resultCountSentence(model) {
  ...
  if (model.route === 'overview') {
    if (model.routingReason === 'unresolved_query') {
      return 'Indexter returned a featured catalog overview because the task was unclear. No task search was attempted.';
    }
    if (model.routingReason === 'input_rejected') {
      return 'Indexter returned a featured catalog overview after rejecting the search input. No task search was attempted.';
    }
    return `Indexter returned ${model.counts.providers} ${model.counts.providers === 1 ? 'provider' : 'providers'} and ${offerings} featured ${offerings === 1 ? 'offering' : 'offerings'}.`;
  }
  if (model.route === 'provider') {
    return offerings === 0
      ? 'Indexter found no current offerings for this provider.'
      : `Indexter returned ${offerings} ${offerings === 1 ? 'offering' : 'offerings'} for this provider.`;
  }
  return offerings === 0
    ? 'Indexter found no current matches for this request.'
    : `Indexter returned ${offerings} ${offerings === 1 ? 'match' : 'matches'} for this request.`;
}

function contentSentence(model) {
  const summary = resultCountSentence(model);
  if (!model.ok) return summary;
  const unavailable = model.results.filter((result) => result.kind === 'endpoint'
    && result.action.reason === 'input_contract_unavailable').length;
  return unavailable === 0 ? summary
    : `${summary} Request details are unavailable for ${unavailable} ${unavailable === 1 ? 'listing' : 'listings'} in this result.`;
}
...
function assembleIndexterToolResult({
  ...
  if (originalQuery !== undefined) {
    model.originalQuery = typeof originalQuery === 'string'
      && originalQuery.length <= 1024
      && isSafeIndexterDiscoveryString(originalQuery)
      ? originalQuery
      : null;
  }
  Object.assign(model, searchScope);
  return {
...
export function buildIndexterToolResult({
  ...
  resolvedQuery,
  routingReason,
  taskSearchAttempted = false,
}) {
  if (!['overview', 'provider', 'task'].includes(route)) {
    throw new TypeError('Unknown Indexter route');
  }
  if (routingReason !== undefined && !ROUTING_REASONS.has(routingReason)) {
    throw new TypeError('Unknown Indexter routing reason');
  }
  // Only the dispatch owner supplies scope. Legacy builders do not infer that
  // a task search occurred from the shape or success of a backend payload.
  const searchScope = routingReason === undefined ? {} : {
    resolvedQuery: typeof resolvedQuery === 'string'
      && resolvedQuery.length <= 1024
      && isSafeIndexterDiscoveryString(resolvedQuery)
      ? resolvedQuery
      : null,
    routingReason,
    taskSearchAttempted: taskSearchAttempted === true,
  };
