...
const modelSafeIndexterDiscoveryEndpointOutput = z.object({
  ...
  const directUrl = value.resourceUrl
    && value.resourceUrl.startsWith('https://')
    && !value.resourceUrl.includes('indexter-managed.invalid');
...
const modelSafeIndexterDiscoveryOutput = z.object({
  ...
  providers: z.array(z.object({
    ...
    catalog: z.object({
      ...
      scannedResourceCount: z.number().int().nonnegative(),
      complete: z.boolean(),
    }).strict(),
    evidence: modelSafeIndexterEvidenceOutput,
  ...
  page: z.object({
    version: z.literal(1),
    order: z.literal('featured_provider_curation_v1'),
