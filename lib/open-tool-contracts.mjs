...
const REGISTRATION_OUTPUT_SCHEMAS = Object.freeze({
  // SDK output normalization accepts a Zod object, not its ZodEffects wrapper.
  // The full stock identity refinements still run in the result policy below.
  [GOVERNED_ASSET_TOOL_NAMES.prepare]:
    GOVERNED_ASSET_TOOL_OUTPUT_SCHEMAS.prepare.innerType(),
...
export function applyOpenToolResultPolicy(toolName, result) {
  ...
  if (
    toolName === GOVERNED_ASSET_TOOL_NAMES.prepare
    && next?.isError !== true
    && next?.structuredContent !== undefined
  ) {
    OUTPUT_SCHEMAS[toolName].parse(next.structuredContent);
  }
