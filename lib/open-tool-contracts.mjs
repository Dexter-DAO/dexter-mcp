...
/**
 * Executable public contract for the cards-off OpenDexter
...
export const OPEN_TOOL_CONTRACTS = Object.freeze({
  ...
  x402_pay: contract({
    name: 'x402_pay',
    title: 'Legacy Paid-Call Alias',
    description:
      'Deprecated compatibility alias for x402_fetch, retained through the next dated compatibility release because current installed callers still use it. Models must use x402_fetch. It preserves the same preparedPurchase, maxAmountAtomic, settlement, replay, and ambiguity rules and may make a real non-idempotent payment.',
    annotations: {
      readOnlyHint: false,
  ...
  x402_compose_skill: contract({
    ...
    title: 'Legacy Skill Composition',
    description:
      'Deprecated compatibility endpoint for the retired composed-skill experiment. New OpenDexter product flows must not route here. Existing marketplace data is preserved, while the tool remains raw-callable only during the dated removal window.',
    annotations: {
      readOnlyHint: false,
      destructiveHint: false,
      idempotentHint: false,
  ...
  promote_skill: contract({
    name: 'promote_skill',
    title: 'Legacy Skill Visibility',
    description:
      'Deprecated compatibility endpoint for visibility on preserved composed-skill records. New OpenDexter product flows must not route here. It remains raw-callable only during the dated removal window and still requires explicit user intent plus ownership.',
    annotations: {
      readOnlyHint: false,
      destructiveHint: true,
      idempotentHint: true,
  ...
  dexter_passkey_probe: contract({
    ...
    title: 'Legacy Passkey Compatibility Probe',
    description:
      'Deprecated app-only diagnostic retained temporarily for installed-client compatibility. It runs a disposable WebAuthn capability check inside the widget sandbox, performs no wallet enrollment or payment, and cannot enable server-side diagnostic telemetry in production. Normal wallet setup uses x402_wallet; models must not route user requests here.',
    annotations: {
      readOnlyHint: false,
      destructiveHint: false,
      idempotentHint: false,
    ...
    // Keep the compatibility endpoint callable by the existing diagnostic
    // widget while removing it from the model-facing product roster.
    visibility: ['app'],
    widgetAccessible: false,
  }),
  dexter_passkey: contract({
    name: 'dexter_passkey',
    title: 'Legacy Dexter Wallet Status',
    description:
      `Deprecated app-only compatibility alias for older wallet widgets; models must use x402_wallet. It makes no payment, but native host OAuth may create or resume setup/session state when the caller is unbound, so the operation is not declared read-only or idempotent. The public deposit field is the receive address; vault_pda and swig_state_address are state/config addresses. ${WALLET_AUTHORITY_SUMMARY}`,
    annotations: {
      readOnlyHint: false,
      destructiveHint: false,
      idempotentHint: false,
    ...
    // Existing widgets can continue calling the alias while new model-facing
    // flows converge on one wallet name and one authority surface.
    visibility: ['app'],
    widgetAccessible: false,
  }),
});
...
export function finalizeOpenToolContracts(server) {
  ...
  const extra = [...observedNames].filter((name) => !OPEN_TOOL_CONTRACTS[name]);
