...
import { createPortfolioReadV3Schemas } from './portfolio-read-v3-contract.mjs';
...
export const PORTFOLIO_READ_V3_SCHEMAS = createPortfolioReadV3Schemas({
  holdingSchema: modelSafePortfolioHoldingOutput,
  targetSchema: modelSafeApprovedActionTargetOutput,
  validateTargets: approvedActionTargetsAreValid,
});
...
const BASE_OUTPUT_SCHEMAS = Object.freeze({
  ...
  dexter_wallet_portfolio: z.object({
    ...
    portfolio: z.union([modelSafePortfolioOutput, PORTFOLIO_READ_SCHEMAS.portfolio, PORTFOLIO_READ_V3_SCHEMAS.portfolio]).optional(),
...
export const OPEN_TOOL_CONTRACTS = Object.freeze({
  ...
  dexter_wallet_portfolio: contract({
    ...
    description:
      'Read the Solana holdings bound to this OpenDexter connection. Fresh reads default to version 3: totals, coverage and up to five holdings, with trading availability not evaluated. Request view=detail with a mint or name/symbol query for exact quantities, price, daily change and available market and identity evidence. Use view=holdings for more rows. Continue a version 3 observation with readVersion=3 and its snapshotId or nextCursor; unversioned saved handles retain version 2. Explore supported assets with view=targets as a separate observation; readVersion=3 plus holdingSnapshotId links it to a holdings observation. Targets include assets outside the holdings shown and carry their own action availability and expiry. source describes the whole observation; selection describes this response. Missing values remain unknown. Names and symbols are display data; assetId identifies the asset. Identity recognition does not grant trading permission; Prepare checks current authority. Expiry requires an explicit new observation. Inputs never select another wallet or grant.',
    annotations: {
      readOnlyHint: true,
      destructiveHint: false,
      idempotentHint: true,
...
const PORTFOLIO_OPAQUE_RESULT_FIELDS = new Set(['assetid', 'snapshotid', 'holdingsnapshotid', 'nextcursor']);
...
export function applyOpenToolResultPolicy(toolName, result) {
  ...
  const selectedPortfolio = toolName === 'dexter_wallet_portfolio'
    && ['opendexter.portfolio.v2', 'opendexter.portfolio.v3'].includes(result?.structuredContent?.portfolio?.contractVersion);
  if (selectedPortfolio) {
    ...
    const schemas = body.portfolio.contractVersion === 'opendexter.portfolio.v3'
      ? PORTFOLIO_READ_V3_SCHEMAS : PORTFOLIO_READ_SCHEMAS;
    schemas.response.parse({ ok: true, portfolio: body.portfolio, card: result._meta?.portfolioCard });
  }
