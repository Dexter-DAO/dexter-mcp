...
const ASSET_NAME = /^[A-Za-z0-9][A-Za-z0-9_.-]*\.(?:css|js)$/;
...
function exactAssetName(value, label) {
  if (typeof value !== 'string' || !ASSET_NAME.test(value)) {
    throw new Error(`${label} is not one exact JS/CSS asset name`);
  }
  return value;
}
...
function referencedAssetNames(html, htmlName) {
  const referenced = [];
  let match;
  while ((match = LOCAL_ASSET_REFERENCE.exec(html)) !== null) {
...
/**
 * Read the complete JS/CSS graph from one already-verified immutable release.
 * HTML is the authority for the subset that must be publicly reachable before
 * the release can serve resources/read responses.
 */
...
/**
 * Append the sealed release's JS/CSS files to the Nginx tree. Existing hashes
...
/** Verify every shipped JS/CSS asset through its exact public URL. */
