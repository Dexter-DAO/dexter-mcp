 ...
 */
export function routeIndexterRequest(value, { providerNames = [], originalQuery } = {}) {
  if (!originalQueryAllowsRouting(originalQuery)) return OVERVIEW_ROUTE;
  const query = normalizeQuery(value);
  ...
  if (isKeywordTask(query)) return TASK_ROUTE;
