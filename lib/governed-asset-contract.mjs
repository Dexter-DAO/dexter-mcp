...
export const GOVERNED_SHARE_QUANTITY_SCHEMA = z.string()
  ...
  .describe(
    'Positive decimal underlying-share-equivalent quantity, such as 10 or 0.25. Do not convert it to token atomic units; Dexter resolves the selected product version, token decimals, and current display multiplier.',
  );
export const GOVERNED_ASSET_ID_SCHEMA = z.string()
  ...
  .describe(
    'Canonical assetId returned by dexter_portfolio from an approved holding or approvedActionTarget. This is not a symbol or mint; Dexter resolves it to one exact network, mint, token program, decimals, and capability record.',
  );
...
const prepareCatalogStockBuySpendInput = z.object({
  ...
  amountAtomic: GOVERNED_POSITIVE_U64_DECIMAL_SCHEMA.describe(
    'Exact USDC budget to spend, in atomic units with 6 decimals, on the stock Dexter selects from the current catalog.',
  ),
  memo: TRADE_MEMO,
  maxSlippageBps: BPS,
  maxPriceImpactBps: BPS,
...
const prepareCatalogStockBuyQuantityInput = z.object({
  ...
  maximumSpendAtomic: GOVERNED_POSITIVE_U64_DECIMAL_SCHEMA.optional().describe(
    'Optional maximum USDC input permitted for this share-quantity Buy, in atomic units with 6 decimals. It is valid only with shareQuantity.',
  ),
  memo: TRADE_MEMO,
  maxSlippageBps: BPS,
  maxPriceImpactBps: BPS,
...
const prepareCatalogStockSellInput = z.object({
  ...
  companyQuery: GOVERNED_COMPANY_QUERY_SCHEMA,
  amountAtomic: GOVERNED_POSITIVE_U64_DECIMAL_SCHEMA.describe(
    'Exact selected-stock token input in atomic units. Dexter resolves the current catalog product, token program, and decimals; never infer them from the company name or symbol.',
  ),
  memo: TRADE_MEMO,
  maxSlippageBps: BPS,
  maxPriceImpactBps: BPS,
...
export const GOVERNED_ASSET_TOOL_CONTRACTS = Object.freeze({
  [GOVERNED_ASSET_TOOL_NAMES.prepare]: descriptor({
    ...
    description:
      'Persist and evaluate one exact Send, Buy, or Sell without signing or submitting it. Tool/schema presence is not runtime capability; only this Prepare response is authoritative. For any natural-language stock Buy or Sell, pass companyQuery and let Dexter resolve and pin the current catalog product; never pass a remembered stock assetId, mint, or symbol. The assetId variants remain for a non-stock approved holding or approvedActionTarget returned by dexter_portfolio. In the current integrated release, covered Buy and Sell may continue autonomously, while Send is preserved in the input contract but fails closed with protected_agent_send_sdk_required before capacity reservation or intent creation. A Buy uses exactly one amount mode: amountAtomic is the exact USDC budget to spend, while shareQuantity is the human decimal underlying-share-equivalent quantity requested and may be paired with maximumSpendAtomic as a USDC ceiling. Never convert shareQuantity to token atomic units; Dexter resolves the exact product version, current display multiplier, and raw minimum output. Never send both modes, and never treat shareQuantity as exact output because slight overfill is possible. Sell and Send amountAtomic are selected-asset input. operationId is only the Idempotency-Key. The server resolves the wallet and reusable bounded mandate; enrollment, extension, or owner escalation happens outside model-callable tools only when the request is not already covered.',
    annotations: {
      readOnlyHint: false,
