...
export const GOVERNED_ASSET_TOOL_CONTRACTS = Object.freeze({
  ...
  [GOVERNED_ASSET_TOOL_NAMES.execute]: descriptor({
    ...
    description:
      'Ask the protected mandate executor to execute one exact intentId returned by a successful Prepare. A request covered by the bound reusable mandate may execute autonomously; an uncovered or runtime-unavailable request fails closed. Never call Execute after protected_agent_send_sdk_required because that refusal creates no executable intent. operationId is only the Idempotency-Key and grants no authority. The tool accepts no plan, attempt, authorization, wallet, agent, grant, mint, or token-program fields. Never retry automatically after a timeout or uncertain result; inspect status and reconcile the same intent.',
    annotations: {
      readOnlyHint: false,
