...
export const GOVERNED_ASSET_ID_SCHEMA = z.string()
  ...
  .describe(
    'Canonical assetId returned by dexter_portfolio from an approved holding or approvedActionTarget. Pass the assetId itself; Dexter resolves its exact network, mint, token program, decimals, and capability record.',
  );
...
export const GOVERNED_ASSET_TOOL_CONTRACTS = Object.freeze({
  [GOVERNED_ASSET_TOOL_NAMES.prepare]: descriptor({
    ...
    description:
      'Persist and evaluate one exact Send, Buy, or Sell without signing or submitting it. Treat this Prepare response as the proof of current runtime capability. For any natural-language stock Buy or Sell, pass companyQuery and let Dexter resolve and pin the current catalog product; never pass a remembered stock assetId, mint, or symbol. The assetId variants remain for a non-stock approved holding or approvedActionTarget returned by dexter_portfolio. In the current integrated release, covered Buy and Sell may continue autonomously, while Send is preserved in the input contract but fails closed with protected_agent_send_sdk_required before capacity reservation or intent creation. A Buy uses exactly one amount mode: amountAtomic is the exact USDC budget to spend, while shareQuantity is the human decimal underlying-share-equivalent quantity requested and may be paired with maximumSpendAtomic as a USDC ceiling. Keep shareQuantity as a human decimal; Dexter resolves the exact product version, current display multiplier, and raw minimum output. Never send both modes, and never treat shareQuantity as exact output because slight overfill is possible. Sell and Send amountAtomic are selected-asset input. operationId is only the Idempotency-Key. The server resolves the wallet and reusable bounded mandate; enrollment, extension, or owner escalation happens outside model-callable tools only when the request is not already covered.',
    annotations: {
      readOnlyHint: false,
