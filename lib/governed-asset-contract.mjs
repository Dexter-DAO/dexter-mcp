...
const prepareBuyQuantityInput = z.object({
  operationId: OPERATION_ID,
  action: z.literal('buy'),
  assetId: GOVERNED_ASSET_ID_SCHEMA,
  shareQuantity: GOVERNED_SHARE_QUANTITY_SCHEMA.describe(
    'Minimum underlying-share-equivalent quantity requested for an approved Solana stock product, expressed as a human decimal such as 10 or 0.25. Dexter resolves the exact product version, current display multiplier, and raw token amount. This is a minimum-receive target and execution may receive slightly more.',
  ),
  maximumSpendAtomic: GOVERNED_POSITIVE_U64_DECIMAL_SCHEMA.optional().describe(
    'Optional maximum USDC input permitted for this share-quantity Buy, in atomic units with 6 decimals. It is valid only with shareQuantity; 1000000 means 1 USDC.',
  ),
  memo: TRADE_MEMO,
  maxSlippageBps: BPS,
  maxPriceImpactBps: BPS,
...
export const GOVERNED_ASSET_TOOL_CONTRACTS = Object.freeze({
  [GOVERNED_ASSET_TOOL_NAMES.prepare]: descriptor({
    ...
    description:
      'Persist and evaluate one exact Send, Buy, or Sell without signing or submitting it. Tool/schema presence is not runtime capability; only this Prepare response is authoritative. In the current integrated release, covered Buy and Sell may continue autonomously, while Send is preserved in the input contract but fails closed with protected_agent_send_sdk_required before capacity reservation or intent creation. Pass only a canonical assetId returned by dexter_portfolio from an approved holding or approvedActionTarget with the requested action available; Dexter resolves and binds its exact network, mint, token program, decimals, and supported actions. A Buy uses exactly one amount mode: amountAtomic is the exact USDC budget to spend, while shareQuantity is the human decimal underlying-share-equivalent quantity requested and may be paired with maximumSpendAtomic as a USDC ceiling. Never convert shareQuantity to token atomic units; Dexter resolves the exact product version, current display multiplier, and raw minimum output. Never send both modes, and never treat shareQuantity as exact output because slight overfill is possible. Sell and Send amountAtomic are selected-asset input. operationId is only the Idempotency-Key. The server resolves the wallet and reusable bounded mandate; enrollment, extension, or owner escalation happens outside model-callable tools only when the request is not already covered.',
    annotations: {
      readOnlyHint: false,
