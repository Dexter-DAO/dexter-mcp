...
export function portfolioResultText(result) {
  ...
  if (portfolio?.contractVersion === 'opendexter.portfolio.v3') {
    const { selection: s, source } = portfolio;
    if (source.kind === 'action_targets') {
      if (s.match === 'unavailable') return 'Supported asset information is unavailable for this observation.';
      return `Showing ${s.returnedCount} of ${source.targetCount} supported ${source.targetCount === 1 ? 'asset' : 'assets'} in this separate observation.`
        + (s.nextCursor ? ' Continue with readVersion 3 and nextCursor.' : ' Action availability applies to this observation.');
    }
    return `Showing ${s.returnedCount} of ${source.holdingCount} observed ${source.holdingCount === 1 ? 'holding' : 'holdings'}.`
      + (s.match === 'ambiguous' ? ' Several holdings match; choose an account for details.' : '')
      + (!source.holdingsComplete ? ' Some holdings could not be read.' : '')
      + ' Trading availability has not been checked.'
      + (s.nextCursor ? ' Continue with readVersion 3 and nextCursor.'
        : ' Use readVersion 3 and this snapshot for details or more holdings.');
  }
  if (portfolio?.contractVersion === 'opendexter.portfolio.v2') {
    const { selection: s, sourceSummary: totals } = portfolio;
    if (s.match === 'unavailable') return 'Approved asset information is unavailable for this observation.';
    const collection = s.view === 'targets' ? 'approved assets' : 'holdings';
    const count = s.view === 'targets' ? totals.targetCount : totals.holdingCount;
    return `Showing ${s.returnedCount} of ${count} observed ${collection}.`
      + (s.match === 'ambiguous' ? ' Several holdings match; choose an account for details.' : '')
      + (!totals.holdingsComplete ? ' The source reports incomplete holdings coverage.' : '')
      + (s.nextCursor ? ' More matching rows are available with nextCursor.' : s.view === 'summary' ? ' Use this snapshot for holding details or more rows.' : '');
  }
  if (result.readError) return result.message;
  if (!portfolio) {
    return JSON.stringify({
