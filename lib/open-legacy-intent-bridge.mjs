...
function replaceLegacyCall(body, target, intentId) {
  const replacement = {
    ...
    params: {
      ...
      arguments: {
        intentId,
        maxAmountAtomic: target.params.arguments.maxAmountAtomic,
      },
    },
  };
  return body === target ? replacement : body;
}
...
export function createLegacyIntentBridge({
  ...
  function recordCheck({ identity, sessionId, modelResult } = {}) {
    ...
    if (existing) {
      if (existing.intentId !== intentId || existing.amountAtomic !== amountAtomic) {
  ...
  function rewriteLegacy(body, { identity, sessionId } = {}) {
    const currentTime = now();
    purge(currentTime);
    const untouched = { body, matched: false, rewritten: false };
    if (Array.isArray(body)) return untouched;
    if (typeof sessionId !== 'string' || !SESSION_ID_RE.test(sessionId)) return untouched;
    const boundIdentity = identityKey(identity);
    if (!boundIdentity) return untouched;
    const legacy = legacyFetchCall(body);
    if (!legacy) return untouched;
    const { message, args } = legacy;
    if (
