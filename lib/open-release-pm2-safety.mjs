...
export const PRODUCTION_PM2_EXECUTABLE =
  '/usr/local/lib/node_modules/pm2/bin/pm2';
export const PRODUCTION_NODE_EXECUTABLE = '/usr/bin/node';

// Keep routine PM2 reads and mutations tightly bounded.
export const DEFAULT_PM2_COMMAND_TIMEOUT_MS = 30_000;

// `pm2 start` and `pm2 resurrect` may wait for the ecosystem's 90-second
// `wait_ready` boundary. Their wrapper needs independent headroom so it cannot
// kill a valid process before PM2's own readiness deadline.
export const DEFAULT_PM2_STARTUP_TIMEOUT_MS = 120_000;
...
export function snapshotPm2ProcessDefinition(row) {
  ...
  if (
    normalizedDefinition.instances === undefined
    || normalizedDefinition.instances === null
    || normalizedDefinition.instances === 1
  ) {
    normalizedDefinition.instances = 1;
  }
...
export async function runBoundedPm2Command({
  ...
  pm2Executable = PRODUCTION_PM2_EXECUTABLE,
  ...
  if (
    ...
    throw new TypeError('Node executable must be the exact production binary');
  }
  if (
    !isAbsolute(pm2Executable)
    || pm2Executable !== PRODUCTION_PM2_EXECUTABLE
  ) {
    throw new TypeError('PM2 executable must be the exact production binary');
  }

  const controller = new AbortController();
  let timeout;
  const operation = Promise.resolve().then(() => runCommand(
    nodeExecutable,
    [pm2Executable, ...args],
    {
      encoding: 'utf8',
