...
import { parseIndexterRequestInputFailure, parseIndexterRequestInputValidation } from './indexter-request-validation.mjs';
...
function publicRequestInputValidation(value, association) {
  if (value?.ok !== true || value.error || Object.hasOwn(value, 'requestInputFailure')
    || ['check_in_progress', 'check_ambiguous', 'check_unknown'].includes(value.status)
    || String(value.authMode || '').toLowerCase() === 'siwx'
    || (value.httpStatus !== undefined
      && (!Number.isInteger(value.httpStatus) || value.httpStatus < 200 || value.httpStatus >= 300))
    || (value.checkRequestId !== undefined && value.checkRequestId !== association.checkRequestId)) return undefined;
  return parseIndexterRequestInputValidation(value.requestInputValidation, association);
}

function publicCheckFields(value, association) {
  if (!value || typeof value !== 'object' || Array.isArray(value)) return {};
  if (value.error === 'indexter_request_input_invalid') {
    const requestInputFailure = parseIndexterRequestInputFailure(value, association.checkRequestId);
    return {
      ok: false,
      error: 'indexter_request_input_invalid',
      retryable: false,
      ...(Number.isInteger(value.httpStatus) ? { statusCode: value.httpStatus } : {}),
      ...(requestInputFailure ? { requestInputFailure } : {}),
    };
  }
  const result = {
  ...
  const requestInputValidation = publicRequestInputValidation(value, association);
  if (requestInputValidation) result.requestInputValidation = requestInputValidation;
  return result;
}
...
export function buildHostedCheckModelResult({
  ...
  const inputRefusal = checkResult?.error === 'indexter_request_input_invalid';
  const intentId =
    !inputRefusal && checkResult?.ok === true
  ...
  const publicFields = publicCheckFields(checkResult, {
    checkRequestId,
    ...(recoveredCheck ? {} : {
      resourceId: mcp === undefined && !url ? resourceId : undefined,
      method: String(validationMethod || 'GET').toUpperCase(), rawBody, rawBodyProvided,
    }),
    recoveredCheck,
  });
  if (!inputRefusal && !publicFields.paymentOptions && checkResult?.paymentRequired === true && typeof checkResult.amountAtomic === 'string') {
  ...
  const schemaResolution = inputRefusal
    ? { replaced: false, source: 'unavailable', rejectedSources: [] }
    : reconcileHostedCheckInputSchema({
...
export function buildHostedCheckStatusModelResult({ checkResult, checkRequestId }) {
  if (checkResult?.error !== 'indexter_request_input_invalid'
    && ((checkResult?.status === 'purchase_already_started' && typeof checkResult.intentId === 'string')
      || checkResult?.delivery || checkResult?.payment)) {
    const requestInputValidation = publicRequestInputValidation(checkResult, { checkRequestId, recoveredCheck: true });
    return {
      ...sanitizeOpenX402IntentResult(checkResult, { httpStatus: checkResult.httpStatus, includeData: false }),
      checkRequestId,
      ...(requestInputValidation ? { requestInputValidation } : {}),
    };
  }
