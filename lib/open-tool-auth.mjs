...
export function registerOpenTool(server, name, config, handler) {
  const securedConfig = withOpenToolAuth(name, config);
  const resourceUri = securedConfig?._meta?.ui?.resourceUri;
  if (typeof resourceUri === 'string' && resourceUri.length > 0) {
    return registerAppTool(server, name, securedConfig, handler);
  }
  return server.registerTool(name, securedConfig, handler);
}
