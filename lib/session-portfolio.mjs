...
export const HOLDING_CAPABILITY_REASON_CODES = Object.freeze([
  ...UNAVAILABLE_GOVERNED_ACTION_REASONS,
  'asset_not_approved',
  'token_program_mismatch',
  'account_state_unverified',
  'unknown',
]);
const HOLDING_REASON_CODES = new Set(HOLDING_CAPABILITY_REASON_CODES);
export const PORTFOLIO_PRICE_SOURCES = Object.freeze([
  'jupiter-price-v3', 'jupiter-exact-in-quote', 'unknown',
]);
  ...
  for (const capability of value) {
    if (
      !hasExactKeys(capability, ['action', 'available', 'reason',
        ...(Object.hasOwn(capability ?? {}, 'reasonCode') ? ['reasonCode'] : [])]) ||
      ...
      (!capability.available && capability.reason === null) ||
      (Object.hasOwn(capability, 'reasonCode') && (
        capability.available
          ? capability.reasonCode !== null
          : capability.reasonCode !== (HOLDING_REASON_CODES.has(capability.reason) ? capability.reason : 'unknown')
      ))
    ) {
      return false;
    }
...
export function portfolioMarketContextIsValid(value, holdingMint) {
  if (value === null) return true;
  return hasExactKeys(value, ['source', 'mint', 'observedAt', 'liquidityUsd', 'holderCount', 'activity24h'])
    && value.source === 'jupiter-tokens-v2'
    && solanaPublicKey(value.mint)
    && value.mint === (holdingMint === 'native:SOL' ? WRAPPED_SOL_MINT : holdingMint)
    && isoDate(value.observedAt)
    && (value.liquidityUsd === null || canonicalDecimal(value.liquidityUsd))
    && (value.holderCount === null || nonNegativeInteger(value.holderCount))
    && hasExactKeys(value.activity24h, ['traderCount'])
    && (value.activity24h.traderCount === null || nonNegativeInteger(value.activity24h.traderCount));
}

export function portfolioRegistryIdentityIsValid(value, approvalStatus) {
  if (value === null) return true;
  return approvalStatus === 'approved'
    && hasExactKeys(value, ['source', 'providerName', 'legalIssuerName'])
    && value.source === 'dexter-registry'
    && [value.providerName, value.legalIssuerName].every((name) =>
      name === null || boundedTrimmedUtf8String(name, 128));
}
...
function validHolding(value) {
  if (!hasExactKeys(value, [...HOLDING_KEYS,
    ...['marketContext', 'registryIdentity'].filter((key) => Object.hasOwn(value ?? {}, key)),
  ])) return false;
  if (
    ...
    (Object.hasOwn(value, 'marketContext') && !portfolioMarketContextIsValid(value.marketContext, value.mint)) ||
    (Object.hasOwn(value, 'registryIdentity') && !portfolioRegistryIdentityIsValid(value.registryIdentity, value.approval?.status))
  ) {
    return false;
  }
...
// Selected reads carry the existing model-safe holding instead of the full
// source row. Keep its numerical invariants without applying full-inventory
// limits or claiming selected rows sum to the whole source value.
export function selectedPortfolioHoldingAmountsAreValid(h) {
  if (!h || !TOKEN_PROGRAMS.has(h.tokenProgram)
    || !(h.mint === 'native:SOL' || solanaPublicKey(h.mint))
    || ((h.tokenProgram === 'native') !== (h.mint === 'native:SOL'))
    || (h.tokenProgram === 'native' ? h.tokenAccount !== null : !solanaPublicKey(h.tokenAccount))
    || typeof h.amountRaw !== 'string' || !/^(0|[1-9][0-9]{0,19})$/.test(h.amountRaw)
    || BigInt(h.amountRaw) > U64_MAX || !nonNegativeInteger(h.decimals) || h.decimals > 255
    || !canonicalDecimal(h.displayAmount) || !AMOUNT_MODELS.has(h.amountModel)
    || (h.symbol !== undefined && !boundedString(h.symbol, 32))
    || (h.name !== undefined && !boundedString(h.name, 128))
    || (h.valueUsd !== null && !canonicalDecimal(h.valueUsd))) return false;
  const multiplier = h.displayMultiplier ?? null;
  if (h.amountModel === 'scaled-ui-amount'
    ? h.tokenProgram !== 'token-2022' || !canonicalDecimal(multiplier) || multiplier === '0'
    : multiplier !== null) return false;
  const rawDisplay = decimalFromRaw(h.amountRaw, h.decimals);
  if (h.displayAmount !== (h.amountModel === 'scaled-ui-amount' ? multiplyDecimals(rawDisplay, multiplier) : rawDisplay)) return false;
  if (h.approvalStatus === 'approved' ? !CANONICAL_ASSET_ID.test(h.assetId ?? '') : h.assetId !== null) return false;
  if (h.approvalStatus === 'blocked' && (h.valueUsd !== null || h.priceUsd !== null || h.availableActions.length !== 0)) return false;
  if (new Set(h.availableActions).size !== h.availableActions.length) return false;
  if (h.priceUsd === null) return h.valueUsd === null && h.priceObservedAt === null
    && (h.change24hPercent ?? null) === null;
  return canonicalDecimal(h.priceUsd) && isoDate(h.priceObservedAt)
    && h.valueUsd === multiplyDecimals(h.displayAmount, h.priceUsd);
}
 ...
 *
 * Bounded holding names and symbols are display data, separate from canonical
 * asset IDs and action availability. Only defined provider context, registry
 * identity, price provenance and controlled capability codes are included.
 * Raw reason text, URLs, registry groups and extension internals stay excluded.
 * Prepare remains the authority decision for the requested action.
 */
export function modelSafePortfolioSnapshot(portfolio) {
  ...
  return {
    ...
    holdings: portfolio.holdings.map((holding) => ({
      ...
      displayMultiplier: holding.displayMultiplier,
      accountState: holding.accountState,
      valueUsd: holding.valueUsd,
      priceUsd: holding.price?.usd ?? null,
      priceObservedAt: holding.price?.observedAt ?? null,
      change24hPercent: holding.price?.change24hPercent ?? null,
      priceSource: holding.price === null ? null
        : PORTFOLIO_PRICE_SOURCES.includes(holding.price.source) ? holding.price.source : 'unknown',
      priceBlockId: holding.price?.blockId ?? null,
      metadataObservedAt: holding.metadataObservedAt,
      ...(Object.hasOwn(holding, 'marketContext') ? { marketContext: holding.marketContext === null ? null : {
        ...holding.marketContext, activity24h: { ...holding.marketContext.activity24h },
      } } : {}),
      ...(Object.hasOwn(holding, 'registryIdentity') ? { registryIdentity: holding.registryIdentity === null ? null : { ...holding.registryIdentity } } : {}),
      approvalStatus: holding.approval.status,
      availableActions: holding.capabilities
        .filter((capability) => capability.available)
        .map((capability) => capability.action),
      capabilities: holding.capabilities.map((capability) => ({
        action: capability.action,
        available: capability.available,
        reasonCode: capability.available ? null : capability.reasonCode ?? 'unknown',
      })),
    })),
...
export async function fetchSessionPortfolio({
  ...
  secret,
  fetchImpl = fetch,
  // The API resolves the wallet, reads stock authority, then reads holdings.
  // Its two RPC stages each have a 7s budget; allow the intervening DB read.
  timeoutMs = 20_000,
}) {
