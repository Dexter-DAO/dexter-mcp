...
export function buildAgentWorkReportModelResult({ input, result }) {
  ...
  const text = acknowledged
    ? `${body.replayed ? 'Original work report recovered' : 'Work report saved'}. `
      + `Agent-reported state: ${body.report.state}. Statement freshness ends at ${body.report.expiresAt}. `
      + (body.replayed ? 'This replay preserves the original report and expiry; a newer report may already be current. ' : '')
      + 'Financial outcomes remain in their transaction receipts.'
