...
export function buildOpenServerInstructions() {
  const upstream = buildServerInstructions({
    ...
    // The composed-skill experiment is no longer part of the OpenDexter
    // product. Compatibility endpoints remain raw-callable for one removal
    // window, but they are absent from model routing.
    hasSkillTools: false,
  });
  return [
