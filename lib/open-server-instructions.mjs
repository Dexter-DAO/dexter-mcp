...
dexter_portfolio reads the exact governed asset inventory bound to that same authenticated session. It accepts no caller-supplied handle, wallet, vault, actor, agent, grant, role, or authority. For governed Send, Buy, or Sell, use only a non-null canonical assetId from an approved holding. Never substitute a symbol or send mint, network, token program, or decimals as authority. availableActions is context, not execution authority.

dexter_prepare_asset_action prepares one exact governed Send, Buy, or Sell without signing or submitting it. Buy amountAtomic is the USDC budget in atomic units (6 decimals). Sell and Send amountAtomic are selected-asset amounts using the server-certified decimals. Send has no memo. operationId is only the Idempotency-Key for an exact replay and grants no authority. prepared plus approval.status=not-required means the reusable bounded mandate covers the request and autonomous Execute is permitted.
