...
const LEGACY_PRIVATE_RELEASE_CONTRACT = Object.freeze({
  ...
  supplementalManifest: Object.freeze({
    root: 'scripts/studio-runtime/node_modules',
    fileCount: 727,
    artifactManifestSize: 95_871,
    artifactManifestSha256:
      'd3b8047d3f18c7ce5765b61812728bc209c302ed39668668edc981c9497d94cb',
  }),
...
function verifyLegacySupplementalManifest(
  releaseDir,
  actual,
  listed,
  contract,
  ...
  exactKeys(contract, [
    'artifactManifestSha256',
    'artifactManifestSize',
    'fileCount',
    'root',
  ], 'legacy supplemental manifest');
  if (
    typeof contract.root !== 'string'
    || contract.root.length === 0
    || path.isAbsolute(contract.root)
    || contract.root.includes('\0')
    || !Number.isSafeInteger(contract.fileCount)
    || contract.fileCount < 1
    || !Number.isSafeInteger(contract.artifactManifestSize)
    || contract.artifactManifestSize < 1
    || typeof contract.artifactManifestSha256 !== 'string'
    || !HEX_64.test(contract.artifactManifestSha256)
  ) {
    throw new Error('legacy supplemental manifest contract is invalid');
  }
  const supplementalRoot = safeReleasePath(releaseDir, contract.root);
  const rootStat = fs.lstatSync(supplementalRoot);
  if (
    !rootStat.isDirectory()
    || rootStat.isSymbolicLink()
    || rootStat.uid !== process.getuid()
    || (rootStat.mode & 0o222) !== 0
    || fs.realpathSync(supplementalRoot) !== supplementalRoot
  ) {
    throw new Error('legacy supplemental manifest root is not exact');
  }
  const prefix = `${contract.root}${path.sep}`;
  if ([...listed.keys()].some((relative) => relative.startsWith(prefix))) {
    throw new Error('legacy supplemental manifest overlaps base manifest');
  }
  const supplemental = actual.filter((relative) => !listed.has(relative));
  if (
    supplemental.length !== contract.fileCount
    || supplemental.some((relative) => !relative.startsWith(prefix))
  ) {
    throw new Error('legacy supplemental manifest file set is not exact');
  }
  const entries = new Map(listed);
  const symlinks = [];
  const lines = [];
  for (const relative of supplemental) {
    const absolute = safeReleasePath(releaseDir, relative);
    ...
    if (before.isFile() && !before.isSymbolicLink()) {
      ...
      after = exactLegacyRegularFile(absolute);
      identity = sha256(fileBytes);
      if (!sameJson(stableLegacyStat(before), stableLegacyStat(after))) {
        throw new Error(`legacy supplemental file changed: ${relative}`);
      }
    } else if (before.isSymbolicLink()) {
      kind = 'L';
      identity = fs.readlinkSync(absolute);
      ...
      if (
        ...
        throw new Error(`legacy supplemental symlink changed: ${relative}`);
      }
      symlinks.push({ absolute, identity, relative });
    } else {
      throw new Error(`unsupported legacy supplemental entry: ${relative}`);
    }
    entries.set(relative, kind);
    metadata.push({
      kind,
      identity,
      relative,
    ...
    lines.push(`${kind}\t${identity}\t${relative}\n`);
  }
  for (const record of symlinks) {
    verifyManifestedSymlink(releaseDir, record, entries);
  }
  const supplementalBytes = Buffer.from(lines.join(''));
  if (
    supplementalBytes.length !== contract.artifactManifestSize
    || sha256(supplementalBytes) !== contract.artifactManifestSha256
  ) {
    throw new Error('legacy supplemental manifest identity mismatch');
  }
  return Object.freeze({
    root: contract.root,
    fileCount: supplemental.length,
    artifactManifestSize: supplementalBytes.length,
    artifactManifestSha256: sha256(supplementalBytes),
  });
}

function verifyLegacyReleaseFileManifest(
  releaseDir,
  bytes,
  ...
  const actual = walkRelease(releaseDir).sort();
  const expected = [...listed.keys()].sort();
  let supplementalIdentity = null;
  if (supplementalManifest === null) {
    if (!sameJson(actual, expected)) {
      throw new Error('legacy release file manifest is incomplete');
    }
  } else {
    supplementalIdentity = verifyLegacySupplementalManifest(
      releaseDir,
      actual,
      listed,
  ...
  if (!sameJson(walkRelease(releaseDir).sort(), actual)) {
    throw new Error('legacy release file manifest is incomplete');
  }
  ...
  return Object.freeze({
    fileCount: listed.size + (supplementalIdentity?.fileCount ?? 0),
    filesystemMetadataSha256: sha256(
      Buffer.from(JSON.stringify(canonicalJson(metadata))),
    ),
    supplementalManifest: supplementalIdentity,
  });
}
...
function readSealedLegacyOpenRelease(
  ...
  const supplementalManifest = contract.supplementalManifest ?? null;
  if (
    supplementalManifest !== null
    && (
      expectedDirectoryName !== `${contract.sourceCommit}-runtime1`
      || manifestDirectoryName !== contract.sourceCommit
      || contract.entrypoint !== 'http-server-oauth.mjs'
      || (
        contract.sourceCommit === LEGACY_OPEN_RELEASE_CONTRACT.sourceCommit
        && !sameJson(
          supplementalManifest,
          LEGACY_PRIVATE_RELEASE_CONTRACT.supplementalManifest,
        )
      )
    )
  ) {
    throw new Error('legacy supplemental manifest is restricted to private alias');
  }
  ...
  return Object.freeze({
    ...
    rollbackIdentity: Object.freeze({
      ...
      ...(verified.supplementalManifest ? {
        supplementalManifest: verified.supplementalManifest,
      } : {}),
    }),
  });
}
