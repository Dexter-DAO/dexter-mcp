...
function knownFailedAction(body) {
  const state = body.business ?? body;
  return state.executionSucceeded === false
    && (body.landingProof === true || state.settlement === 'landed')
    && ['confirmed', 'finalized'].includes(state.finality ?? body.confirmationCommitment);
}
...
function actionPresentation(body) {
  ...
  if (knownFailedAction(body)) {
    result.summary = 'The transaction landed, but the action failed.';
    result.nextActions = [{ tool: 'dexter_asset_action_status', arguments: { intentId: body.intentId },
      reason: 'The action failed. Read this same intent only if a later receipt or finality update is needed.' }];
    return result;
  }
...
function successfulFinalityStep(body) {
  if (body.reconciliationRequired === true && body.canReconcile === true) {
    return { tool: 'dexter_reconcile_asset_action', arguments: { intentId: body.intentId },
      ...
      reason: 'The action has succeeded and is complete. Finalization can be recorded separately on this same transaction.' };
  }
  return { tool: 'dexter_asset_action_status', arguments: { intentId: body.intentId },
    reason: 'The action has succeeded and is complete. A later read of this same transaction can observe its finality update.' };
}
...
export function presentGovernedAgentResult(body) {
  ...
  if (body.statusAfter) {
    ...
    const failedPendingFinality = body.outcome === 'pending' && knownFailedAction(body.statusAfter);
    if (body.outcome === 'pending') {
      presentation.nextActions = [{ tool: 'dexter_asset_action_status', arguments: { intentId: body.intentId },
        reason: successfulPendingFinality
          ? 'The action has succeeded and is complete. Its finality update is pending; a later read can observe this same transaction.'
          : failedPendingFinality
            ? 'The action failed. Its finality update is pending; a later read can observe this same transaction.'
            : 'Recovery remains uncertain. Inspect this same attempt; do not automatically repeat Reconcile.' },
      ...presentation.nextActions.filter((next) => next.tool === 'dexter_wallet_portfolio')];
    }
    return { ...presentation, recoveryOutcome: body.outcome,
      recoveryMessage: successfulPendingFinality
        ? 'The action has succeeded and is complete. Its transaction finality update is pending.'
        : failedPendingFinality
          ? 'The action failed. Its transaction finality update is pending.'
