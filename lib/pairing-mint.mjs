...
export async function mintPairingRequest(scope = 'dextercard') {
  const url = internalApiUrl('/api/connector/oauth/authorize');
  url.searchParams.set('client_id', CLIENT_ID);
  url.searchParams.set('redirect_uri', REDIRECT_URI);
  url.searchParams.set('scope', scope);
  url.searchParams.set('response_mode', 'json');

  const res = await fetchInternalApi(`${url.pathname}${url.search}`, {
    method: 'GET',
...
export async function pollPairingResult(requestId) {
  ...
  const path = `/api/connector/oauth/result?request_id=${encodeURIComponent(requestId)}`;
  const res = await fetchInternalApi(path, {
    method: 'GET',
    headers: {
...
// This is the RETURN target (post-bind), passed to pair/authorize as redirect_uri
// (flows to authorize:87 → the pairing's stored redirect_uri, NOT the enroll page).
// The enroll/login URL is a SEPARATE constant on the dexter-api side (VAULT_ENROLL_URL,
// default /wallet/setup-passkey). Do NOT conflate: pointing this at the done page is
// correct and does not affect where the user enrolls.
...
export async function mintVaultPairingRequest(mcpSessionId = null) {
  const url = internalApiUrl('/api/passkey-vault/pair/authorize');
...
export async function pollVaultPairingResult(requestId) {
  ...
  const path = `/api/passkey-vault/pair/result?request_id=${encodeURIComponent(requestId)}`;
  const res = await fetchInternalApi(path, {
    method: 'GET',
...
export async function fetchVaultStateBySession(mcpSessionId, opts = {}) {
  if (!SECRET) throw new Error('INTERNAL_DEXTERCARD_HMAC_SECRET missing');
  const ts = String(Date.now());
  const sig = createHmac('sha256', SECRET).update(`${ts}.${mcpSessionId}`).digest('hex');
  // opts.money: ask for the credit-line + carry composition (extra RPC reads
  // server-side) — the wallet dashboard wants it, pay paths don't.
  const path = `/api/passkey-vault/state?mcp_session_id=${encodeURIComponent(mcpSessionId)}${opts.money ? '&money=1' : ''}`;
  // MUST have a timeout: the money=1 dashboard read makes server-side Solana RPC
  // reads, and under RPC rate-limiting (429 → backoff) those can stack into an
  // effective hang. Without this, dexter_wallet waits forever and the tool call
  // times out client-side instead of failing fast into buildVaultReadError
  // ("couldn't reach your wallet, funds safe, retry"). Sibling
  // fetchVaultStateByUserHandle already does this; this one silently didn't.
  // Wallet and portfolio reads allow the API's bounded provider fallback to
  // finish. Portfolio needs this budget without requesting extra money reads.
  // Payment lookups retain their shorter deadline.
  const res = await fetchInternalApi(path, {
    headers: { 'x-internal-timestamp': ts, 'x-internal-signature': sig },
    signal: AbortSignal.timeout(opts.money || opts.portfolio ? 8000 : 3000),
  });
...
export async function fetchVaultStateByUserHandle(userHandleBase64) {
  ...
  const path = `/api/passkey-vault-anon/status?user_handle=${encodeURIComponent(b64url)}`;
  const res = await fetchInternalApi(path, { signal: AbortSignal.timeout(3000) });
