...
export const OPEN_X402_CHECK_DESCRIPTION = 'Check one exact HTTP request or MCP tool call before paying. Choose url, an Indexter resourceId, or mcp. For resourceId, copy the method from the current Indexter result and requestInputVersion:2 when its requestInput is version 2. For an HTTP body, retain the exact raw JSON string. For mcp, first call x402_mcp_tools, copy its schema string verbatim, and serialize the requested arguments once. A purchasable check returns quoteOnly=false and an intentId for x402_fetch and x402_status. Checking may invoke a tool or change provider state. Payment requires an approved spending ceiling; use the same intent to inspect an uncertain purchase.';
...
export const openX402CheckShape = {
  ...
  requestInputVersion: z.union([z.literal(1), z.literal(2)]).optional().describe('For resourceId only. Copy 2 from an Indexter version 2 requestInput to validate the body against the current catalog contract.'),
};
...
export function refineCheckTarget(value, context) {
  ...
  if (value.requestInputVersion !== undefined && value.resourceId === undefined) {
    context.addIssue({ code: z.ZodIssueCode.custom, path: ['requestInputVersion'], message: 'Request input version requires resourceId' });
  }
  if (value.requestInputVersion === 2 && value.method === 'GET') {
    context.addIssue({ code: z.ZodIssueCode.custom, path: ['method'], message: 'Version 2 requires POST, PUT or DELETE' });
  }
