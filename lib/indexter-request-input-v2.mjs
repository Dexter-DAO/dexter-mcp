...
function safeDeclaredField(field) {
  const name = field.name;
  if (!safeIdentifier(name)) return false;
  if (name === 'prompt' || !UNSAFE_NAME.test(name)) return true;
  // Declared-name policy v2: one inert prefix and a closed string selector.
  const selector = /^([A-Za-z][A-Za-z0-9]*)_system$/.exec(name);
  return selector !== null && safeIdentifier(selector[1]) && !UNSAFE_NAME.test(selector[1])
    && field.type === 'string' && Object.hasOwn(field, 'enum')
    && Array.isArray(field.enum) && field.enum.length > 0
    && field.enum.every((value) => typeof value === 'string'
      && /^[A-Za-z][A-Za-z0-9_-]{0,127}$/.test(value)
      && isSafeIndexterObjectKey(value) && isSafeIndexterDiscoveryString(value)
...
export function parseIndexterRequestInputV2(value) {
  ...
  function fields(values, nested) {
    ...
    for (const field of values) {
      if (!isRecord(field) || ++count > 24 || !safeIdentifier(field.name)
      ...
      if (field.type === 'object') {
        if (nested || !keysEqual(field, [...keys, 'additionalProperties', 'fields'])
          || field.additionalProperties !== false || !safeDeclaredField(field)) return null;
        ...
        if (nested || !keysEqual(field, [...keys, 'items', 'minItems', 'maxItems'])
          ...
          || !Number.isInteger(field.maxItems) || field.maxItems < field.minItems || field.maxItems > 32 || !safeDeclaredField(field)) return null;
        ...
        if (!safeDeclaredField(field)) return null;
...
// COMMON-WIRE.v1 / API pure-helper.v4: classify before selecting the legacy parser.
...
export function projectIndexterRequestInputV2(schema) {
  ...
  function fields(value, nested) {
    ...
    for (const name of names.sort(compare)) {
      if (!safeIdentifier(name) || ++count > 24) return null;
