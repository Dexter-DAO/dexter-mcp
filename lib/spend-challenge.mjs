...
/**
 * Protected tools that retain the raw HTTP OAuth fallback for clients making
 * a direct or cached call before OAuth promotion. The governed reads share
 * the same wallet/mandate identity boundary as governed execution. General
 * wallet/passkey reads still reach their handler so modern hosts receive the
 * tool-level `_meta["mcp/www_authenticate"]` challenge.
 */
export const SPEND_TOOL_NAMES = new Set([
  'x402_fetch',
  ...REGISTERED_GOVERNED_ASSET_TOOL_NAMES,
]);
