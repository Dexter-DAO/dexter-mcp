...
import { GOVERNED_RECEIPT_OUTCOME_SCHEMA, receiptOutcomeMatchesAction } from './governed-receipt-outcome.mjs';
import { presentGovernedAgentResult } from './governed-agent-presentation.mjs';
import { createGovernedPresentationSchemas } from './governed-presentation-contract.mjs';
import { GOVERNED_USD_VALUE_BINDING_SCHEMA, usdValueMatchesAsset } from './governed-usd-value.mjs';
...
const NONZERO_HASH = HASH.refine((value) => !/^0{64}$/.test(value));
...
function decodedBase58ByteLength(value) {
  ...
  const decodedMagnitudeBytes = bytes.length === 1 && bytes[0] === 0
    ? 0
    : bytes.length;
  return leadingZeroBytes + decodedMagnitudeBytes;
}

function canonicalSolanaAddress(value) {
  return decodedBase58ByteLength(value) === 32;
}

function canonicalSolanaSignature(value) {
  return decodedBase58ByteLength(value) === 64;
}

// Preserve the current public API's bounded address surface for legacy and
// non-stock envelopes. Catalog stock selection pins independently require a
// canonical 32-byte Solana public key below.
const ADDRESS = z.string().min(32).max(64);
const CANONICAL_SOLANA_ADDRESS = z.string()
  .min(32)
  .max(44)
  .refine(canonicalSolanaAddress);
// The API can expose a bounded durable signature before the attempt is final.
// Canonical 64-byte Solana encoding is therefore a success invariant below,
// not a blanket schema rule that would break legacy non-stock reads.
const BOUNDED_SIGNATURE = z.string().min(1).max(256);
...
const CANONICAL_STOCK_SHARE = z.string()
  .max(39)
  .regex(/^(?:0|[1-9][0-9]{0,19})(?:\.([0-9]{0,17}[1-9]))?$/)
  .refine((value) => value !== '0');
const COMPANY_QUERY = z.string().min(1).max(256);

const productIdentityShape = {
  assetId: ASSET,
...
function validateProductIdentity(value, context) {
  if (
    ...
    && (
      ...
      || value.legalIssuerName.length === 0
    ...
    context.addIssue({
      ...
      path: ['providerName'],
      message: 'stock identity requires distinct provider and legal issuer fields',
    });
  }
  if (
    typeof value.legalIssuerName === 'string'
...
const productIdentity = z.object(productIdentityShape).strict()
  .superRefine(validateProductIdentity);

const PREVIEW_AMOUNT_FIELDS = [
  'amountModel', 'displayMultiplier', 'amountObservedAtUnixMs', 'amountObservedAtSlot',
];
const PREVIEW_MULTIPLIER = z.string().max(128)
  .regex(/^(?:0|[1-9][0-9]*)(?:\.[0-9]{0,63}[1-9])?$/)
  .refine((value) => value !== '0');
const PREVIEW_OBSERVATION_SLOT = z.string().max(16).regex(/^[1-9][0-9]*$/)
  .refine((value) => Number.isSafeInteger(Number(value)) && Number(value) > 0);

// Quote observations are optional only for retained previews created before
// this metadata was introduced. Immutable trade summaries retain their schema.
const previewProductIdentity = z.object({
  ...productIdentityShape,
  amountModel: z.enum(['raw-decimals', 'scaled-ui-amount', 'unknown']).optional(),
  displayMultiplier: PREVIEW_MULTIPLIER.nullable().optional(),
  amountObservedAtUnixMs: z.number().int().min(0).max(Number.MAX_SAFE_INTEGER).optional(),
  amountObservedAtSlot: PREVIEW_OBSERVATION_SLOT.optional(),
}).strict().superRefine(validateProductIdentity).superRefine((value, context) => {
  if (!PREVIEW_AMOUNT_FIELDS.some((field) => Object.hasOwn(value, field))) return;
  if (PREVIEW_AMOUNT_FIELDS.some((field) => value[field] === undefined)
    || (value.amountModel === 'unknown' && value.displayMultiplier !== null)
    || (value.amountModel === 'raw-decimals' && value.displayMultiplier !== '1')
    || (value.amountModel === 'scaled-ui-amount'
      && (value.displayMultiplier === null || value.tokenProgram !== 'token-2022'))) {
    context.addIssue({ code: z.ZodIssueCode.custom, path: ['amountModel'],
      message: 'preview amount observation must be complete and match the amount model' });
  }
});
...
const jupiterServiceFee = z.object({
  version: z.literal('dexter-jupiter-service-fee/v1'),
  rateBps: z.literal(75),
  side: z.enum(['input', 'output']),
  mint: CANONICAL_SOLANA_ADDRESS,
  tokenProgram: z.enum(['spl-token', 'token-2022']),
  feeAccount: CANONICAL_SOLANA_ADDRESS,
  owner: CANONICAL_SOLANA_ADDRESS,
  quotedFeeAmountAtomic: U64,
}).strict();

function validateQuotedServiceFee(value, context) {
  if (value.serviceFee !== undefined && (value.platformFee === null
    || value.platformFee.mint !== value.serviceFee.mint
    || value.platformFee.amountAtomic !== value.serviceFee.quotedFeeAmountAtomic)) {
    context.addIssue({ code: z.ZodIssueCode.custom, path: ['platformFee'],
      message: 'platform fee must match the quoted service fee' });
  }
}

// Preserve the established non-stock Prepare envelope. Stock outputs carry a
// narrower, identity-bound summary validated independently below.
const preparedFeeSummary = z.object({
  summary: z.literal(
    'Trading fees are included in this quote; network fee is calculated at execution.',
  ),
  platformFee: preparedFeeLine.nullable(),
  serviceFee: jupiterServiceFee.optional(),
  routeFees: z.array(preparedFeeLine).max(32),
  networkFee: z.object({
    status: z.literal('not-yet-calculated'),
    amountLamports: z.null(),
  }).strict(),
}).strict().superRefine(validateQuotedServiceFee);

const stockFeeSummary = z.object({
  summary: z.literal(
    'Trading fees are included in this quote; network fee is calculated at execution.',
  ),
  platformFee: preparedFeeLine.nullable(),
  serviceFee: jupiterServiceFee.optional(),
  routeFees: z.array(preparedFeeLine).max(32).superRefine((fees, context) => {
    for (let index = 1; index < fees.length; index += 1) {
      if (fees[index - 1].mint >= fees[index].mint) {
        context.addIssue({
          code: z.ZodIssueCode.custom,
          path: [index, 'mint'],
          message: 'route fees must be strictly mint-sorted without duplicates',
        });
      }
    }
  }),
...
}).strict().superRefine(validateQuotedServiceFee).superRefine((value, context) => {
  if (value.serviceFee === undefined && value.platformFee !== null) {
    context.addIssue({ code: z.ZodIssueCode.custom, path: ['platformFee'],
      message: 'stock platform fee requires the quoted service fee' });
  }
});

export const GOVERNED_STOCK_TRADE_SUMMARY_NAMESPACE =
  'dexter-governed-stock-trade-summary/v1';
export const GOVERNED_STOCK_PREPARE_SUMMARY_SNAPSHOT_NAMESPACE =
  'dexter-governed-stock-prepare-summary-snapshot/v1';

export const GOVERNED_STOCK_TRADE_SUMMARY_SCHEMA = z.object({
  namespace: z.literal(GOVERNED_STOCK_TRADE_SUMMARY_NAMESPACE),
  action: z.enum(['buy', 'sell']),
  assetId: ASSET,
  symbol: z.string().min(1).max(32),
  amountAtomic: POSITIVE_U64,
  requestAmountKind: z.enum(['input', 'share-quantity', 'usd-value']),
  usdValue: GOVERNED_USD_VALUE_BINDING_SCHEMA.optional(),
  requestedShareQuantity: CANONICAL_STOCK_SHARE.nullable(),
  expectedShareQuantity: CANONICAL_STOCK_SHARE.nullable(),
  minimumShareQuantity: CANONICAL_STOCK_SHARE.nullable(),
  shareQuantityUnit: z.literal('underlying-share-equivalent').nullable(),
  shareQuantitySemantics: z.literal('minimum-receive').nullable(),
  requestedMaximumSpendAtomic: POSITIVE_U64.nullable(),
  overfillPossible: z.boolean(),
  productIdentity,
  feeSummary: stockFeeSummary,
}).strict().superRefine((value, context) => {
  if (
    value.productIdentity.assetClass !== 'stock'
    || value.productIdentity.assetId !== value.assetId
    || value.productIdentity.symbol !== value.symbol
  ) {
    context.addIssue({
      code: z.ZodIssueCode.custom,
      path: ['productIdentity'],
      message: 'trade summary product identity must match the stock asset',
    });
  }

  if (value.requestAmountKind === 'usd-value'
    ? !usdValueMatchesAsset(value.usdValue, value.action, value.amountAtomic, value.productIdentity)
    : value.usdValue !== undefined) {
    context.addIssue({ code: z.ZodIssueCode.custom, path: ['usdValue'],
      message: 'USD value evidence must match the Sell amount and product' });
  }

  if (value.requestAmountKind === 'share-quantity') {
    if (
      value.action !== 'buy'
      || value.requestedShareQuantity === null
      || value.expectedShareQuantity === null
      || value.minimumShareQuantity === null
      || value.shareQuantityUnit !== 'underlying-share-equivalent'
      || value.shareQuantitySemantics !== 'minimum-receive'
      || value.overfillPossible !== true
    ) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ['requestAmountKind'],
        message: 'share-quantity summary is incomplete or contradictory',
      });
      return;
    }
    if (
      !decimalQuantityAtLeast(
        value.minimumShareQuantity,
        value.requestedShareQuantity,
      )
      || !decimalQuantityAtLeast(
        value.expectedShareQuantity,
        value.minimumShareQuantity,
      )
      || (
        value.requestedMaximumSpendAtomic !== null
        && BigInt(value.amountAtomic)
          > BigInt(value.requestedMaximumSpendAtomic)
      )
    ) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ['minimumShareQuantity'],
        message: 'share-quantity summary violates its receive or spend bounds',
      });
    }
    return;
  }
  ...
  if (
    ...
    || value.expectedShareQuantity !== null
    || value.minimumShareQuantity !== null
    || value.shareQuantityUnit !== null
    || value.shareQuantitySemantics !== null
    || value.requestedMaximumSpendAtomic !== null
    || value.overfillPossible
  ) {
    context.addIssue({
      code: z.ZodIssueCode.custom,
      path: ['requestAmountKind'],
      message: 'input summary must not claim share-quantity request terms',
    });
  }
});

const LOWER_UUID = z.string().regex(
  /^[0-9a-f]{8}-[0-9a-f]{4}-[1-8][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/,
);
const CANONICAL_SEQUENCE = z.string()
  .regex(/^(0|[1-9][0-9]*)$/)
  .refine((value) => BigInt(value) <= 9_223_372_036_854_775_807n);
const CANONICAL_DISPLAY_TEXT = (maximum) => z.string()
  .min(1)
  .max(maximum)
  .refine((value) => (
    value === value.trim()
...
export const GOVERNED_STOCK_SELECTION_SCHEMA = z.object({
  namespace: z.literal('dexter-governed-stock-selection-pin/v1'),
  normalizedCompanyQuery: CANONICAL_DISPLAY_TEXT(256),
  companyName: CANONICAL_DISPLAY_TEXT(256),
  productName: CANONICAL_DISPLAY_TEXT(256),
  productSymbol: CANONICAL_DISPLAY_TEXT(64),
  providerName: CANONICAL_DISPLAY_TEXT(256),
  legalIssuerName: CANONICAL_DISPLAY_TEXT(256),
  underlyingId: LOWER_UUID,
  underlyingIdentityDigest: HASH,
  selectionEventId: LOWER_UUID,
  selectionSequence: CANONICAL_SEQUENCE,
  selectionDigest: HASH,
  selectedVariantId: LOWER_UUID,
  assetId: ASSET,
  productVersionId: LOWER_UUID,
  productVersionDigest: HASH,
  onchainVersionId: LOWER_UUID,
  onchainVersionDigest: HASH,
  executionProfileVersionId: LOWER_UUID,
  executionProfileDigest: HASH,
  executionReleaseId: LOWER_UUID,
  releaseDigest: HASH,
  registryIdentityDigest: HASH,
  mint: CANONICAL_SOLANA_ADDRESS,
  tokenProgram: z.enum(['spl-token', 'token-2022']),
  ...
  if (normalizeCompanyQuery(value.normalizedCompanyQuery) !== value.normalizedCompanyQuery) {
    context.addIssue({
      code: z.ZodIssueCode.custom,
      path: ['normalizedCompanyQuery'],
      message: 'stock selection query must already be canonical',
    });
  }
});

export const GOVERNED_STOCK_V2_IDENTITY_SCHEMA = z.object({
  namespace: z.literal('dexter-governed-stock-v2-durable-identity/v1'),
  runtimeReleaseDigest: NONZERO_HASH,
  intentId: LOWER_UUID,
  requestIdentityHash: NONZERO_HASH,
  intentHash: NONZERO_HASH,
  preparedPlanHash: NONZERO_HASH,
  coordinateDigest: NONZERO_HASH,
  planDigest: NONZERO_HASH,
  attestationId: LOWER_UUID,
  prepareDraftDigest: NONZERO_HASH,
  tradeSummarySnapshotDigest: NONZERO_HASH,
  requestClaimDigest: NONZERO_HASH,
  executionCapabilityReceiptDigest: NONZERO_HASH,
}).strict();

const stockPrepareRuntime = z.object({
  namespace: z.literal('dexter-delegated-stock-prepare-runtime-binding/v2'),
  runtimeReleaseDigest: NONZERO_HASH,
  requestClaimDigest: NONZERO_HASH,
  durableIdentityDigest: NONZERO_HASH,
}).strict();
...
const stockCategoryAuthority = z.object({
  authorityNamespace: z.literal('stock-category-v2'),
  categoryRuleId: UUID,
  categoryRuleDigest: NONZERO_HASH,
  grantBindingId: UUID,
  apiRuntimeAdmissionDigest: NONZERO_HASH,
  attestorVerifierTrustSetDigest: NONZERO_HASH,
  graphConfigAccount: ADDRESS,
  graphConfigAccountDataSha256: NONZERO_HASH,
}).strict();
...
const attribution = z.object({
  ...
  wallet,
  grant: z.union([z.object({
    authorityNamespace: z.literal('grant-rule-v1').optional(),
    id: UUID,
  ...
  }).strict(), stockCategoryAuthority.extend({
    id: UUID, revision: z.number().int().positive().max(Number.MAX_SAFE_INTEGER),
    revisionDigest: HASH, riskPolicyDigest: HASH.optional(),
    validFrom: TIMESTAMP, expiresAt: TIMESTAMP.nullable(),
  }).strict()]),
}).strict();

const business = z.object({
  action: ACTION,
  assetId: ASSET.nullable(),
  requestedCompanyQuery: COMPANY_QUERY.optional(),
  amountAtomic: POSITIVE_U64.nullable(),
...
const preview = z.object({
  ...
  usdValue: GOVERNED_USD_VALUE_BINDING_SCHEMA.optional(),
  requestedShareQuantity: CANONICAL_STOCK_SHARE.nullable().optional(),
  expectedShareQuantity: CANONICAL_STOCK_SHARE.nullable().optional(),
  minimumShareQuantity: CANONICAL_STOCK_SHARE.nullable().optional(),
  ...
  productIdentity: previewProductIdentity,
  stockSelection: GOVERNED_STOCK_SELECTION_SCHEMA.optional(),
  feeSummary: preparedFeeSummary,
  shareQuantityConversion: z.object({
    ...
    displayMultiplier: CANONICAL_STOCK_SHARE,
  ...
  if (product.amountModel !== undefined) {
    const usd = value.usdValue;
    const conversion = value.shareQuantityConversion;
    if (usd !== undefined && (product.amountModel !== usd.amountModel
      || product.displayMultiplier !== usd.multiplier.value
      || product.amountObservedAtUnixMs !== usd.mintObservedAtUnixMs
      || product.amountObservedAtSlot !== String(usd.mintObservedSlot))) {
      context.addIssue({ code: z.ZodIssueCode.custom, path: ['productIdentity'],
        message: 'preview amount observation must match the frozen USD conversion' });
    }
    if (conversion != null && (product.decimals !== conversion.rawOutputDecimals
      || product.amountObservedAtSlot !== conversion.multiplierObservedAtSlot
      || product.displayMultiplier !== conversion.displayMultiplier
      || product.amountModel !== (conversion.multiplierSource === 'identity'
        ? 'raw-decimals' : 'scaled-ui-amount')
      || (conversion.multiplierEffectiveAtUnixMs !== null
        && conversion.multiplierEffectiveAtUnixMs
          > Math.floor(product.amountObservedAtUnixMs / 1000) * 1000))) {
      context.addIssue({ code: z.ZodIssueCode.custom, path: ['productIdentity'],
        message: 'preview amount observation must match the frozen share conversion' });
    }
  }
  if (value.requestAmountKind === 'usd-value'
    ? !usdValueMatchesAsset(value.usdValue, value.action, value.amountAtomic, value.productIdentity)
      || value.inputMint !== value.usdValue?.mint
      || value.outputMint !== SOLANA_USDC_MINT
    : value.usdValue !== undefined) {
    context.addIssue({ code: z.ZodIssueCode.custom, path: ['usdValue'],
      message: 'USD value evidence must match the Sell preview' });
  }
});
...
const preparedResponse = z.object({
  ...
  stockRuntime: stockPrepareRuntime.optional(),
  preview,
  ...
  const stock = value.preview.productIdentity.assetClass === 'stock';
  if (stock !== (value.stockRuntime !== undefined)) {
    context.addIssue({
      code: z.ZodIssueCode.custom,
      path: ['stockRuntime'],
      message: 'current catalog stock Prepare requires one runtime binding',
    });
  }
  if (stock !== (value.preview.stockSelection !== undefined)) {
    context.addIssue({
      code: z.ZodIssueCode.custom,
      path: ['preview', 'stockSelection'],
      message: 'current catalog stock Prepare requires one frozen selection',
    });
  }
  if (!stock) return;
  const selection = value.preview.stockSelection;
  const product = value.preview.productIdentity;
  if (
    selection === undefined
    || value.account === null
    || value.business.assetId !== selection.assetId
    || value.business.requestedCompanyQuery !== selection.normalizedCompanyQuery
    || value.preview.assetId !== selection.assetId
    || value.preview.symbol !== selection.productSymbol
    || product.assetId !== selection.assetId
    || product.companyName !== selection.companyName
    || product.productName !== selection.productName
    || product.symbol !== selection.productSymbol
    || product.providerName !== selection.providerName
    || product.legalIssuerName !== selection.legalIssuerName
    || product.issuer !== selection.legalIssuerName
    || product.mint !== selection.mint
    || product.tokenProgram !== selection.tokenProgram
    || product.decimals !== selection.decimals
    || product.registryIdentityDigest !== selection.registryIdentityDigest
    || !stockFeeSummary.safeParse(value.preview.feeSummary).success
    || value.preview.amountAtomic !== value.business.amountAtomic
    || value.preview.maximumInputAmountAtomic !== value.business.amountAtomic
    || (
      value.preview.action === 'buy'
        ? value.preview.inputMint !== SOLANA_USDC_MINT
          || value.preview.outputMint !== selection.mint
        : value.preview.inputMint !== selection.mint
          || value.preview.outputMint !== SOLANA_USDC_MINT
    )
  ) {
    context.addIssue({
      code: z.ZodIssueCode.custom,
      path: ['preview', 'stockSelection'],
      message: 'stock Prepare selection and public identity must be exact',
    });
  }
});
...
const prepareRefusedResponse = z.object({
  ...
  permissionRequest: z.object({
    namespace: z.literal('dexter-stock-permission-request/v1'), family: z.literal('stocks'),
    requestId: z.string().regex(/^vspr_[a-f0-9]{36}$/),
    approvalUrl: z.string().url(), expiresAt: TIMESTAMP,
  }).strict().superRefine((value, ctx) => {
    const url = new URL(value.approvalUrl);
    const policyDigest = url.searchParams.get('policy_digest');
    const expectedKeys = policyDigest === null ? ['request_id'] : ['policy_digest', 'request_id'];
    if (url.origin !== 'https://dexter.cash' || url.pathname !== '/tabs/setup' ||
        url.username || url.password || url.hash ||
        url.searchParams.get('request_id') !== value.requestId ||
        [...url.searchParams.keys()].sort().join('\0') !== expectedKeys.join('\0') ||
        url.searchParams.toString() !== url.search.slice(1) ||
        (policyDigest !== null && !NONZERO_HASH.safeParse(policyDigest).success)) {
      ctx.addIssue({ code: z.ZodIssueCode.custom, message: 'stock approval must bind the exact Dexter request' });
    }
  }).optional(),
  code: REASON,
...
const executeResponse = z.object({
  receiptOutcome: GOVERNED_RECEIPT_OUTCOME_SCHEMA.optional(),
  ...
  transactionSignature: BOUNDED_SIGNATURE.nullable(),
  executed: z.boolean(),
  code: REASON.nullable(),
  explanation: z.string().min(1).max(1_024).nullable(),
  attribution,
  business,
  tradeSummary: GOVERNED_STOCK_TRADE_SUMMARY_SCHEMA.optional(),
...
export const GOVERNED_TRANSACTION_STATUS_SCHEMA = z.object({
  ...
  stockSelection: GOVERNED_STOCK_SELECTION_SCHEMA.optional(),
  tradeSummary: GOVERNED_STOCK_TRADE_SUMMARY_SCHEMA.optional(),
  stockV2Identity: GOVERNED_STOCK_V2_IDENTITY_SCHEMA.optional(),
  amountAtomic: POSITIVE_U64,
  destinationOwner: ADDRESS.nullable(),
  protocolId: PROTOCOL_ID,
  wallet,
  ...
  authorityIdentity: z.union([stockCategoryAuthority,
    z.object({ authorityNamespace: z.literal('grant-rule-v1'), grantRuleId: UUID }).strict(),
    z.object({ authorityNamespace: z.literal('owner') }).strict(),
  ]).optional(),
...
const reconcileResponse = z.object({
  ...
  code: z.enum([
    ...
    'stock_vault_v2_recovery_unavailable',
  ]).nullable(),
  explanation: z.string().min(1).max(1_024),
...
const programTransitionHoldResponse = z.object({
  status: z.literal('paused'),
  code: z.literal('VAULT_PROGRAM_TRANSITION_HELD'),
  message: z.literal('Vault operations are paused for a program update. Retry the same operation after maintenance.'),
  retryable: z.literal(true),
  retryWithSameRequestOnly: z.literal(true),
}).strict();

const programTransitionHoldBody = z.discriminatedUnion('operation', [
  programTransitionHoldResponse.extend({
    namespace: z.literal('opendexter-governed-maintenance/v1'),
    operation: z.literal('prepare'), operationId: BOUNDED_ID, intentId: z.null(),
  }).strict(),
  programTransitionHoldResponse.extend({
    namespace: z.literal('opendexter-governed-maintenance/v1'),
    operation: z.literal('execute'), operationId: BOUNDED_ID, intentId: UUID,
  }).strict(),
  programTransitionHoldResponse.extend({
    namespace: z.literal('opendexter-governed-maintenance/v1'),
    operation: z.literal('reconcile'), operationId: z.null(), intentId: UUID,
  }).strict(),
]);
...
const presentations = createGovernedPresentationSchemas({ feeSummary: preparedFeeSummary });
export const GOVERNED_PRESENTED_ERROR_SCHEMAS = Object.freeze(Object.fromEntries(
  Object.keys(GOVERNED_ASSET_TOOL_OUTPUT_SCHEMAS).map((operation) => [operation, z.object({
    presentation: operation === 'history'
      ? z.union([presentations.error, presentations.history])
      : presentations.error,
  }).strict()])));

export function governedDetailedBody(value) {
  if (!value || typeof value !== 'object' || Array.isArray(value)) return value;
  const { presentation: _presentation, ...body } = value;
  return body;
}

function projectedJson(body) {
  // Match the existing JSON text representation, including omitted undefined fields.
  return JSON.parse(JSON.stringify(presentGovernedAgentResult(body)));
}

export function governedPresentationMatchesBody(presentation, body) {
  try {
    return canonicalHash(presentation) === canonicalHash(projectedJson(body));
  } catch {
    return false;
  }
}

function presentedResultSchemas(backendSchema, presentationSchema) {
  const backendObject = backendSchema.innerType?.() ?? backendSchema;
  const registration = backendObject.extend({ presentation: presentationSchema }).strict();
  const exact = registration.superRefine((value, context) => {
    const body = governedDetailedBody(value);
    if (!backendSchema.safeParse(body).success) {
      context.addIssue({ code: z.ZodIssueCode.custom, message: 'governed result evidence is inconsistent' });
      return;
    }
    if (!governedPresentationMatchesBody(value.presentation, body)) {
      context.addIssue({ code: z.ZodIssueCode.custom, path: ['presentation'],
        message: 'governed presentation must match the detailed result' });
    }
  });
  return { registration, exact };
}

const presentedSchemas = Object.fromEntries(Object.entries(GOVERNED_ASSET_TOOL_OUTPUT_SCHEMAS)
  .map(([operation, schema]) => [operation, presentedResultSchemas(schema,
    operation === 'history' ? presentations.history : presentations.action)]));

// Backend response validation stays separate from the additive MCP projection.
export const GOVERNED_PRESENTED_SUCCESS_SCHEMAS = Object.freeze(Object.fromEntries(
  Object.entries(presentedSchemas).map(([operation, schema]) => [operation, schema.exact])));
export const GOVERNED_PRESENTED_REGISTRATION_SCHEMAS = Object.freeze(Object.fromEntries(
  Object.entries(presentedSchemas).map(([operation, schema]) => [operation, schema.registration])));
export const GOVERNED_PRESENTED_OUTPUT_SCHEMAS = Object.freeze(Object.fromEntries(
  Object.entries(presentedSchemas).map(([operation, schema]) => [operation,
    z.union([schema.exact, GOVERNED_PRESENTED_ERROR_SCHEMAS[operation]])])));
...
export function isGovernedPresentationErrorBody(operation, body) {
  if (!GOVERNED_ASSET_OPERATIONS.includes(operation)) return false;
  if (body?.namespace === 'opendexter-governed-maintenance/v1') {
    return programTransitionHoldBody.safeParse(body).success && body.operation === operation;
  }
  if (body?.namespace === 'opendexter-governed-backend-failure/v1') {
    return GOVERNED_LOCAL_FAILURE_SCHEMA.safeParse(body).success
      && body.operation === operation;
  }
  if (!GOVERNED_BACKEND_RESPONSE_SCHEMAS[operation].safeParse(body).success) return false;
  if (body.namespace === 'dexter-governed-agent-http-refusal/v1') return true;
  if (operation === 'prepare') return body.status !== 'prepared';
  if (operation === 'execute') return body.status !== 'confirmed';
  if (operation === 'reconcile') return body.outcome === 'unavailable';
  return true;
}
...
function backendResultIsError(operation, httpStatus, body) {
  if (operation === 'reconcile' && httpStatus === 409 && body.outcome === 'not-required') return false;
  ...
  if (operation === 'reconcile') {
    return body.outcome === 'unavailable';
  }
  return false;
}

function exactExecuteBusinessIdentity(state) {
  if (state.assetId === null || state.amountAtomic === null) return false;
...
function exactTradeSummaryBusinessIdentity(summary, state) {
  if (summary === undefined) return true;
  return summary.action === state.action
    && summary.assetId === state.assetId
    && summary.amountAtomic === state.amountAtomic;
}

function exactTradeSummaryStatusIdentity(summary, status) {
  if (summary === undefined) return true;
  return exactTradeSummaryBusinessIdentity(summary, status)
    && summary.productIdentity.mint === status.assetMint
    && summary.productIdentity.tokenProgram === status.tokenProgram;
}

export function governedStockPrepareSummarySnapshot(summary) {
  const product = summary.productIdentity;
  return {
    namespace: GOVERNED_STOCK_PREPARE_SUMMARY_SNAPSHOT_NAMESPACE,
    action: summary.action,
    assetId: summary.assetId,
    symbol: summary.symbol,
    amountAtomic: summary.amountAtomic,
    requestAmountKind: summary.requestAmountKind,
    ...(summary.usdValue ? { usdValue: structuredClone(summary.usdValue) } : {}),
...
export function governedStockTradeSummarySnapshotDigest(summary) {
  return canonicalHash(governedStockPrepareSummarySnapshot(summary));
}

function exactStockTradeSummarySnapshotIdentity(summary, identity) {
  if (identity === undefined) return true;
  try {
    return governedStockTradeSummarySnapshotDigest(summary)
      === identity.tradeSummarySnapshotDigest;
  } catch {
    return false;
  }
}

function expectedProductRefusalHttpStatus(code) {
  if (code === 'owner_approval_required' || code === 'idempotency_conflict') {
    return 409;
  }
  if (code === 'invalid_request' || code === 'invalid_delegated_execute_request') {
    return 400;
  }
  return /^stock_[a-z0-9._:-]*(?:unavailable|invalid|mismatch)$/.test(
    code ?? '',
  )
    ? 503
    : 422;
...
function exactPrepareResponse(httpStatus, body) {
  if (body.namespace === 'dexter-governed-agent-http-refusal/v1') {
    return httpStatus >= 400 && httpStatus <= 599;
  }
  if (body.status === 'prepared') return httpStatus === 200;
  if (body.status === 'uncertain') return httpStatus === 503;
  return httpStatus === expectedProductRefusalHttpStatus(body.code);
}

function exactStockStatusIdentity(
  status,
  { allowMissingStockV2Identity = false } = {},
) {
  const selection = status.stockSelection;
  const summary = status.tradeSummary;
  const identity = status.stockV2Identity;
  const selectionBearingSuccess = selection !== undefined
    && status.status === 'confirmed'
    && status.ledgerState === 'confirmed'
    && status.executionSucceeded === true;
  if (
    selectionBearingSuccess
    && identity === undefined
    && !allowMissingStockV2Identity
  ) return false;
  if (
    identity !== undefined
    && (selection === undefined || summary === undefined)
  ) return false;
  if (
    selection !== undefined
    && (
      !['buy', 'sell'].includes(status.action)
      || selection.assetId !== status.assetId
      || selection.mint !== status.assetMint
      || selection.tokenProgram !== status.tokenProgram
    )
  ) return false;
  if (
    summary !== undefined
    && !exactTradeSummaryStatusIdentity(summary, status)
  ) return false;
  if (
    selection !== undefined
    && summary !== undefined
    && (
      summary.symbol !== selection.productSymbol
      || summary.productIdentity.assetId !== selection.assetId
      || summary.productIdentity.companyName !== selection.companyName
      || summary.productIdentity.productName !== selection.productName
      || summary.productIdentity.symbol !== selection.productSymbol
      || summary.productIdentity.providerName !== selection.providerName
      || summary.productIdentity.legalIssuerName !== selection.legalIssuerName
      || summary.productIdentity.issuer !== selection.legalIssuerName
      || summary.productIdentity.mint !== selection.mint
      || summary.productIdentity.tokenProgram !== selection.tokenProgram
      || summary.productIdentity.decimals !== selection.decimals
      || summary.productIdentity.registryIdentityDigest
        !== selection.registryIdentityDigest
    )
  ) return false;
  return identity === undefined
    || (
      identity.intentId === status.intentId
      && exactStockTradeSummarySnapshotIdentity(summary, identity)
    );
}
...
function exactExecuteResponse(httpStatus, body) {
  ...
  if (
    ...
    || !exactTradeSummaryBusinessIdentity(body.tradeSummary, state)
  ...
  if (body.status === 'confirmed') {
    ...
    return httpStatus === 200
      && attemptPresent
      && signaturePresent
      && (!body.executed || canonicalSolanaSignature(body.transactionSignature))
  ...
  if (body.status === 'pending') {
    ...
    const stockRecoveryUnavailable =
      body.code === 'stock_vault_v2_recovery_unavailable';
    return httpStatus === (stockRecoveryUnavailable ? 503 : 202)
      && attemptPresent
      && !body.executed
      && body.explanation !== null
      ...
      && (
        claimed
          ? !signaturePresent
            && [
              'claimed_attempt_resume_adapter_required',
              'stock_vault_v2_recovery_unavailable',
            ].includes(body.code)
  ...
  if (body.status === 'refused') {
    const expectedHttpStatus = expectedProductRefusalHttpStatus(body.code);
    if (
...
function exactGovernedAgentAuthority(status) {
  const identity = status.authorityIdentity;
  if (identity?.authorityNamespace === 'stock-category-v2' &&
      (!['buy', 'sell'].includes(status.action) || !status.stockSelection || !status.stockV2Identity)) return false;
  if (identity?.authorityNamespace === 'grant-rule-v1' && identity.grantRuleId !== status.grantRuleId) return false;
  if (identity?.authorityNamespace === 'owner') return false;
  if (
    status.actor !== 'agent'
    ...
    || (status.authorityIdentity?.authorityNamespace === 'stock-category-v2'
...
function exactGovernedAgentStatusCrossFields(status, options) {
  if (
    !exactGovernedActionIdentity(status)
    || !exactAssetProgramIdentity(status)
    || !exactGovernedAgentAuthority(status)
    || !exactTradeSummaryStatusIdentity(status.tradeSummary, status)
    || !exactStockStatusIdentity(status, options)
  ) {
    return false;
  }
  ...
  if (landed) {
    if (
      ...
      || (
        status.executionSucceeded === true
        && !canonicalSolanaSignature(status.transactionSignature)
      )
...
function exactReconcileResponse(httpStatus, body) {
  ...
  if (body.statusAfter.receiptOutcome !== undefined) {
    const { digest, ...evidence } = body;
    if (canonicalHash(evidence) !== digest) return false;
  }
  ...
  if (
    ...
    || !exactGovernedAgentStatusCrossFields(body.statusAfter, {
      allowMissingStockV2Identity:
        body.code === 'stock_public_identity_required',
    })
  ) {
    return false;
  }
  ...
  if (
    ...
    && (
      ...
      || (
        body.code === 'stock_vault_v2_recovery_unavailable'
        && (
          body.statusAfter.stockSelection === undefined
          || body.statusAfter.tradeSummary === undefined
          || body.statusAfter.stockV2Identity === undefined
        )
      )
      || (
        body.code === 'stock_public_identity_required'
...
function backendResponseMatchesInput(operation, input, body) {
  ...
  if (operation === 'prepare') {
    const expectedDestination = input.action === 'send'
      ...
      : null;
    const catalogStock = typeof input.companyQuery === 'string';
    const normalizedQuery = catalogStock
      ? normalizeCompanyQuery(input.companyQuery)
      : null;
    const quantityTarget = input.action === 'buy'
      && input.shareQuantity !== undefined;
    const usdValueTarget = input.action === 'sell' && input.valueUsd !== undefined;
    if (
      ...
      || (
        catalogStock
          ? body.business.requestedCompanyQuery !== (
            body.status === 'prepared' ? normalizedQuery : input.companyQuery
          )
          : body.business.requestedCompanyQuery !== undefined
      )
      || (
        !catalogStock
      ...
      || (
        !quantityTarget && !usdValueTarget
        && body.business.amountAtomic !== input.amountAtomic
      )
      || (
        (quantityTarget || usdValueTarget)
        && body.status !== 'prepared'
        && body.business.amountAtomic !== null
      )
    ) {
      return false;
    }
    if (body.status !== 'prepared') return true;
    const selection = body.preview.stockSelection;
    if (
      ...
      || (
        catalogStock
          ? selection === undefined
            || selection.normalizedCompanyQuery !== normalizedQuery
            || body.preview.assetId !== selection.assetId
            || body.business.assetId !== selection.assetId
          : selection !== undefined
    ...
    if (usdValueTarget) {
      return body.preview.requestAmountKind === 'usd-value'
        && (catalogStock || body.preview.productIdentity.assetId === input.assetId)
        && body.preview.usdValue?.requestedValueUsd === input.valueUsd
        && body.preview.amountAtomic === body.business.amountAtomic
        ...
        && body.preview.overfillPossible === false
        && body.preview.shareQuantityConversion === null
        && body.preview.expectedOutputAtomic !== null
        && body.preview.minimumOutputAtomic !== null
        && BigInt(body.preview.expectedOutputAtomic) >= BigInt(body.preview.minimumOutputAtomic);
    }
    if (!quantityTarget) {
      if (body.preview.amountAtomic !== input.amountAtomic) return false;
      if (catalogStock) {
        return body.preview.requestAmountKind === 'input'
          && body.preview.requestedShareQuantity === null
          && body.preview.minimumShareQuantity === null
          && body.preview.expectedShareQuantity === null
          && body.preview.maximumInputAmountAtomic === input.amountAtomic
          && body.preview.requestedMaximumSpendAtomic === null
          && body.preview.shareQuantityUnit === null
          && body.preview.shareQuantitySemantics === null
          && body.preview.overfillPossible === false
      ...
      const rawSwap = body.preview.productIdentity.assetId === input.assetId
        && (input.action === 'buy'
          ? body.preview.inputMint === SOLANA_USDC_MINT
            && body.preview.outputMint === body.preview.productIdentity.mint
          : input.action === 'sell'
            && body.preview.inputMint === body.preview.productIdentity.mint
            && body.preview.outputMint === SOLANA_USDC_MINT);
      return (body.preview.requestAmountKind === undefined
        ...
        && (body.preview.maximumInputAmountAtomic === undefined
          ...
          || (rawSwap && body.preview.maximumInputAmountAtomic === input.amountAtomic))
        ...
        && (body.preview.overfillPossible === undefined
          || (rawSwap && body.preview.overfillPossible === false))
  ...
  if (operation === 'execute') {
    return body.requestId === input.operationId
      ...
      && receiptOutcomeMatchesAction(body);
  }
  if (operation === 'status' || operation === 'reconcile') {
    return body.intentId === input.intentId
      && receiptOutcomeMatchesAction(operation === 'reconcile' ? body.statusAfter ?? body : body)
      && (
        operation === 'reconcile'
  ...
  if (operation === 'history') {
    return body.items.length <= (input.limit ?? 25)
      && body.items.every((item) => exactGovernedAgentStatusCrossFields(item) && receiptOutcomeMatchesAction(item));
  }
  return false;
}
...
export function normalizeGovernedAssetResult({
  ...
  if (httpStatus === 503 && ['prepare', 'execute', 'reconcile'].includes(operation)) {
    const held = programTransitionHoldResponse.safeParse(body);
    if (held.success) {
      return {
        body: { ...held.data, namespace: 'opendexter-governed-maintenance/v1',
          operation, ...operationIds(operation, input) },
        httpStatus,
        isError: true,
      };
    }
  }
  if (body?.code === 'VAULT_PROGRAM_TRANSITION_HELD') {
    return buildGovernedAssetFailure({ operation, input, code: 'governed_backend_response_invalid' });
  }
  ...
  if (
    ...
    || (
      operation === 'prepare'
      && !exactPrepareResponse(httpStatus, parsed.data)
    )
    || (
      operation === 'execute'
...
export function buildGovernedAssetToolResult(result, meta = {}) {
  ...
  const widgetOnlyError = result.isError === true && !landedProgramError
    ? { 'dexter/governedWidgetResult': body }
    : {};
  const presentation = projectedJson(body);
  (Array.isArray(body.items) ? presentations.history : presentations.action).parse(presentation);
  return {
    ...
    ...(result.isError === true && !landedProgramError
      ...
      : { structuredContent: { ...body, presentation } }),
    isError: result.isError === true,
    // Ordinary errors expose only the same human projection as the text.
    // Their detailed validated body remains in the widget-only channel.
    _meta: { ...meta, ...widgetOnlyError },
  };
}
