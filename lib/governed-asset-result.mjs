...
function finalReconciliationStatus(status) {
  return (
    (status.status === 'confirmed'
      && status.confirmationCommitment === 'finalized')
    || status.definitiveNonlandingProof === true
...
function exactReconcileResponse(httpStatus, body) {
  ...
  const final = finalReconciliationStatus(body.statusAfter);
  if (
  ...
  if (
    ...
    && (
      ...
      || ['claimed', 'signed'].includes(body.statusAfter.status)
      || final
    )
  ) {
    return false;
