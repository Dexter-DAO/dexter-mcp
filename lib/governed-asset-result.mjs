...
function hasExactNonExecutionEvidence(body) {
  return (
    body?.executed === false
...
function normalizedStatus({
  ...
  if (
    ...
    && (
      phase !== 'execute'
      || hasExactNonExecutionEvidence(body)
    )
  ) {
    return 'refused';
  }
...
export function normalizeGovernedAssetResult({
  ...
  // `false` is an assertion that execution did not reach that stage. Preserve
  // uncertainty as null unless the normalized status proves a definite state.
  const executionUnknown = phase === 'execute' && status === 'unknown';
  ...
  return {
    ...
    execution: {
      ...
      confirmed: executionUnknown ? null : status === 'confirmed',
      transactionSignature:
