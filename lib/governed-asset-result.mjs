...
export function buildGovernedAssetToolResult(result, meta = {}) {
  ...
  const widgetOnlyError = result.isError === true && !landedProgramError
    ...
    : {};
  return {
    content: [{ type: 'text', text: JSON.stringify(presentGovernedAgentResult(body), null, 2) }],
    ...(result.isError === true && !landedProgramError
      ? {}
      : { structuredContent: body }),
    isError: result.isError === true,
    // Ordinary governed errors remain text-only to the model, while the
    // renderer receives the already-validated body through the widget-only
    // metadata channel. This lets refusal and recovery states render without
    // weakening the public result policy.
