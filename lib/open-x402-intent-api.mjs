...
import { purchaseResultGuidance, readablePurchaseAmount } from './open-purchase-result.mjs';
import { fetchInternalApi } from './internal-api-fetch.mjs';
import { nativeMcpServerUrlSchema, nativeMcpTargetSchema } from './native-mcp-contract.mjs';
...
export const OPEN_X402_INTENT_API_PATHS = Object.freeze({
  ...
  checkStatus: '/v2/pay/anon/x402/check/status',
  fetch: '/v2/pay/anon/x402/fetch',
  status: '/v2/pay/anon/x402/status',
  mcpTools: '/v2/pay/anon/x402/mcp/tools',
});

const SESSION_ID_RE = /^[A-Za-z0-9_.\-]{1,256}$/;
const REQUEST_ID_RE = /^[A-Za-z0-9][A-Za-z0-9._:-]{0,127}$/;
export const OPEN_X402_INTENT_ID_RE =
  /^[0-9a-f]{8}-[0-9a-f]{4}-[1-8][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/;
export const OPEN_INDEXTER_RESOURCE_ID_RE =
  /^[0-9a-f]{8}-[0-9a-f]{4}-[1-8][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/;
...
function opaqueIntentId(value) {
  if (
    typeof value !== 'string'
    || !OPEN_X402_INTENT_ID_RE.test(value)
  ) {
...
export function buildOpenX402IntentRequest(action, input = {}) {
  if (action === 'mcpTools') {
    return { serverUrl: nativeMcpServerUrlSchema.parse(input.serverUrl) };
  }
  const mcpSessionId = requiredSessionId(input.sessionId);
  if (action === 'checkStatus') {
    return { mcp_session_id: mcpSessionId, requestId: exactCheckRequestId(input.checkRequestId) };
  }
  if (action === 'check') {
    if (Object.prototype.hasOwnProperty.call(input, 'mcp')) {
      if (['url', 'resourceId', 'method', 'body', 'requestInputVersion'].some((key) => Object.prototype.hasOwnProperty.call(input, key))) {
        throw new TypeError('native_mcp_target_exclusive');
      }
      return { mcp_session_id: mcpSessionId, requestId: exactCheckRequestId(input.requestId),
        mcp: nativeMcpTargetSchema.parse(input.mcp) };
    }
    const hasUrl = typeof input.url === 'string' && input.url.length > 0;
    const hasResourceId = typeof input.resourceId === 'string'
      && input.resourceId.length > 0;
    if (hasUrl === hasResourceId) {
      throw new TypeError('exactly_one_x402_check_target_required');
    }
    if (
      hasResourceId
      && !OPEN_INDEXTER_RESOURCE_ID_RE.test(input.resourceId)
    ) {
      throw new TypeError('invalid_indexter_resource_id');
    }
    if (hasResourceId && input.method === undefined) {
      throw new TypeError('indexter_resource_method_required');
    }
    const body = exactBody(input.body);
    const method = canonicalMethod(input.method);
    const version = input.requestInputVersion;
    if (Object.hasOwn(input, 'requestInputVersion') && (!hasResourceId || ![1, 2].includes(version))) {
      throw new TypeError('invalid_indexter_request_input_version');
    }
    if (version === 2 && method === 'GET') throw new TypeError('indexter_request_input_method_unsupported');
    return {
      mcp_session_id: mcpSessionId,
      requestId: exactCheckRequestId(input.requestId),
      ...(hasUrl ? { url: input.url } : { resourceId: input.resourceId }),
      method,
      ...(body === undefined ? {} : { body }),
      ...(version === undefined ? {} : { requestInputVersion: version }),
    };
  }
  if (action === 'fetch') {
    if (
...
function publicDelivery(value) {
  ...
  return {
    ...(value.transport === 'mcp' ? { transport: 'mcp',
      ...(typeof value.toolError === 'boolean' ? { toolError: value.toolError } : {}) } : {}),
...
function publicPayment(value) {
  ...
  return {
    ...
    ...(['confirmed', 'finalized'].includes(value.commitment) ? { commitment: value.commitment } : {}),
    ...(typeof value.asset === 'string' ? { asset: value.asset } : {}),
    ...(typeof value.network === 'string' ? { network: value.network } : {}),
    ...(Number.isInteger(value.decimals) && value.decimals >= 0 && value.decimals <= 18 ? { decimals: value.decimals } : {}),
    ...(readablePurchaseAmount(value) ? { readableAmount: readablePurchaseAmount(value) } : {}),
...
export function sanitizeOpenX402IntentResult(
  ...
  const result = {
    ...
    ...(publicRouteNeutralDetail(source.reason)
      ...
      : {}),
    ...(Number.isSafeInteger(source.retryAfterMs) && source.retryAfterMs >= 0 ? { retryAfterMs: source.retryAfterMs } : {}),
    ...(typeof source.replacementAllowed === 'boolean' ? { replacementAllowed: source.replacementAllowed } : {}),
  ...
  return { ...purchaseResultGuidance(result), ...result };
}
