...
test('unified wallet activity shows receipts, exact amounts, paging and read failures on desktop and mobile', async (t) => {
  const vite = await createServer({ root: path.join(root, 'apps-sdk/ui'), plugins: [react()], server: { host: '127.0.0.1', port: 0 }, logLevel: 'error' });
  await vite.listen();
  ...
  for (const mobile of [false, true]) {
    await t.test(mobile ? 'mobile' : 'desktop', async () => {
      ...
      await page.waitForFunction(() => !document.querySelector('.dxw-activity-refresh')?.disabled);
      assert.equal(await page.getByText('−0.001 USDC', { exact: true }).count(), 1);
      await page.getByRole('button', { name: /SYRAA.fun market analysis/ }).click();
      await page.getByText('Delivery: Response received (HTTP 200)').waitFor();
      await page.getByRole('button', { name: 'View transaction', exact: true }).click();
      assert.deepEqual(await page.evaluate(() => window.__openedLinks), ['https://solscan.io/tx/fixture']);
