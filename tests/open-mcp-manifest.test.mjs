...
test('well-known manifest advertises one stable twelve-tool OAuth contract', () => {
  ...
  assert.deepEqual(manifest.rosters.anonymous, OPEN_TOOL_NAMES);
  assert.deepEqual(manifest.rosters.oauthPromotes, []);
...
test('hosted source documentation states the current opaque-intent product contract', async () => {
  ...
  assert.match(readme, /exact discoverable roster/i);
