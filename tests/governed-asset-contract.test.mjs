...
test('operation identity never substitutes for authority or owner approval', () => {
  ...
  assert.match(execute, /current task and active mandate cover it/);
  assert.match(execute, /without another approval question/);
  assert.match(execute, /unavailable or uncovered request cannot execute/);
  assert.match(execute, /Do not repeat Execute/);
  assert.match(reconcile, /one existing intent/);
  assert.match(reconcile, /already-signed transaction/);
  assert.match(reconcile, /status-only request/);
  assert.match(reconcile, /automatically repeating/);
});
