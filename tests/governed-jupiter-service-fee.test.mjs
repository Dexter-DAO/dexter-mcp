...
import { normalizeGovernedAssetResult, buildGovernedAssetToolResult,
  ...
  governedDetailedBody, GOVERNED_PRESENTED_OUTPUT_SCHEMAS,
...
function envelope(operation, input, body, httpStatus = operation === 'reconcile' && body.outcome === 'pending' ? 202 : 200) {
  const result = buildGovernedAssetToolResult(normalizeGovernedAssetResult({ operation, input, body, httpStatus }));
...
function reject(f, label) {
  ...
  assert.deepEqual(Object.keys(r.structuredContent), ['presentation'], label);
  assert.equal(r._meta['dexter/governedWidgetResult'].code, 'governed_backend_response_invalid', label);
...
test('actual stock and token Buy/Sell fee fragments cross the real client and Prepare envelope unchanged', async () => {
  for (const c of API.cases) {
    ...
    assert.deepEqual(governedDetailedBody(result.structuredContent), f.prepared, c.name);
    assert.deepEqual(result.structuredContent.presentation, JSON.parse(result.content[0].text));
    const shown = JSON.parse(result.content[0].text);
    ...
    assert.deepEqual(governedDetailedBody(envelope('prepare', f.input, replay).structuredContent), replay);
  }
});
...
test('fee-absent historical Prepare and stock summaries keep their exact old shape and snapshot identity', () => {
  for (const name of ['nvidia', 'tesla']) {
    ...
    const p = f.prepared.preview;
    assert.deepEqual(governedDetailedBody(envelope('prepare', f.input, f.prepared).structuredContent), f.prepared);
...
test('saved stock fees survive Execute, Status, History and Reconcile without becoming actual receipt fees', () => {
  for (const buy of [false, true]) {
    ...
    for (const [op, input, body] of [
      ...
      assert.deepEqual(governedDetailedBody(shown.structuredContent), body);
