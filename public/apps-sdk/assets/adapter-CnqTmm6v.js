...
let _mcpTheme = "dark";
let _mcpHostContext = createDefaultHostContext();
let _mcpHostCapabilities = createDefaultCapabilities();
...
function getChatGptHostContext() {
  ...
  const maxHeight = typeof openai?.maxHeight === "number" && Number.isFinite(openai.maxHeight) ? openai.maxHeight : void 0;
  return stableSnapshot("chatgpt-host-context", {
    theme: openai?.theme ?? "dark",
    displayMode,
    availableDisplayModes: typeof openai?.requestDisplayMode === "function" ? Array.from(/* @__PURE__ */ new Set([displayMode, "inline", "fullscreen"])) : [displayMode],
    ...maxHeight ? { containerDimensions: { maxHeight } } : {},
    locale: openai?.locale,
    platform: openai?.userAgent?.device?.type === "mobile" ? "mobile" : openai?.userAgent?.device?.type === "desktop" ? "desktop" : void 0,
...
function useToolResponseMetadata() {
  return reactExports.useSyncExternalStore(
    HOST === "chatgpt" ? (onChange) => subscribeChatGPT("toolResponseMetadata", onChange) : HOST === "mcp-apps" ? (onChange) => subscribeMcpApps(onChange) : () => () => {
    },
    HOST === "chatgpt" ? () => window.openai?.toolResponseMetadata ?? null : HOST === "mcp-apps" ? () => _mcpToolMeta : () => null,
    () => null
  );
}
...
function useAdaptiveHostContext() {
  return reactExports.useSyncExternalStore(
    HOST === "chatgpt" ? subscribeChatGPTGlobals : HOST === "mcp-apps" ? subscribeMcpApps : () => () => {
    },
    HOST === "chatgpt" ? getChatGptHostContext : HOST === "mcp-apps" ? () => _mcpHostContext : createDefaultHostContext,
...
function useAdaptiveHostCapabilities() {
  return reactExports.useSyncExternalStore(
    ...
    HOST === "chatgpt" ? getChatGptCapabilities : HOST === "mcp-apps" ? () => _mcpHostCapabilities : createDefaultCapabilities,
...
export {
  ...
  openLink as o,
  useToolInput as p,
  useAdaptiveUpdateModelContext as q,
  reactExports as r,
  useAdaptiveSendFollowUp as s,
  useAdaptiveCallToolFn as t,
  useToolOutput as u,
  commonjsGlobal as v
};
