...
const SENSITIVE_KEY = /(?:^|_)(?:address|authorization|body|cookie|credential|email|id|input|name|output|password|payload|phone|prompt|query|request|response|secret|session|token|user|wallet)(?:$|_)/i;
const URL_KEY = /(?:url|uri|href|endpoint|origin)/i;
const SAFE_ENUM = /^[a-z0-9_.:-]{1,96}$/i;
function safeTelemetryUrl(rawUrl) {
...
function safeTelemetryError(error2) {
  const name = typeof error2?.name === "string" && SAFE_ENUM.test(error2.name) ? error2.name : "Error";
  const code = typeof error2?.code === "string" && SAFE_ENUM.test(error2.code) ? error2.code : null;
...
function sanitizeTelemetryValue(value, key = "", depth = 0) {
  ...
  if (typeof value === "string") {
    const origin = URL_KEY.test(normalizedKey) || /^https?:\/\//i.test(value) ? safeTelemetryUrl(value) : null;
...
function sanitizeTelemetryRecord(value) {
  const clean = sanitizeTelemetryValue(value);
  return clean && typeof clean === "object" && !Array.isArray(clean) ? clean : { value: clean };
}
function getRuntime() {
...
function addWidgetBreadcrumb(message, data) {
  addBreadcrumb({
    ...
    message,
    level: "info",
    data: data ? sanitizeTelemetryRecord(data) : void 0
  });
}
function captureWidgetException(error2, extras) {
  const summary = safeTelemetryError(error2);
  captureMessage(`widget_exception_${summary.name}`, {
    level: "error",
...
function initWidgetSentry() {
  ...
  setContext("widget_runtime", sanitizeTelemetryRecord({
...
try {
  ...
  try {
    console.warn("[dexter-apps-sdk] widget telemetry initialization failed", error2);
  } catch {
  }
}
...
function isToolResultContent(value) {
  return isRecord$1(value) && typeof value.type === "string";
}
...
function normalizeCallToolResult(value) {
  ...
  const content = Array.isArray(value.content) ? value.content.filter(isToolResultContent) : void 0;
  const structuredContent = value.structuredContent;
  const textContent = content?.find(
    (item) => item.type === "text" && typeof item.text === "string"
  )?.text;
  const result = typeof value.result === "string" ? value.result : textContent ?? (structuredContent !== void 0 ? stringifyResult(structuredContent) : stringifyResult(value));
  return {
    ...value,
    ...content ? { content } : {},
    result
...
const DEXTER_TOOL_INVOCATION_META_KEY = "dexter/toolInvocation";
const OPENAI_WIDGET_SESSION_META_KEY = "openai/widgetSessionId";
const TOOL_RESULT_ATTACH_TIMEOUT_MS = 3e4;
...
function cleanWidgetSessionId(value) {
  return cleanString(value);
}
...
function invocationKey(identity) {
...
function sameIdentity(expected, actual) {
  ...
  if (expected.widgetSessionId && actual.widgetSessionId && actual.widgetSessionId !== expected.widgetSessionId) return false;
  return true;
}
function compatibleIdentity(left, right) {
  ...
  if (left.widgetSessionId && right.widgetSessionId && left.widgetSessionId !== right.widgetSessionId) return false;
  return true;
}
function textContentOutput(content) {
...
function resultOutput(result) {
  ...
  if (result.structuredContent !== null && result.structuredContent !== void 0) {
    return result.structuredContent;
  }
  return textContentOutput(result.content);
}
function widgetSessionFromRecord(value) {
  if (!value) return null;
  const nestedOpenAi = isRecord(value.openai) ? value.openai : null;
  return cleanWidgetSessionId(value[OPENAI_WIDGET_SESSION_META_KEY]) ?? cleanWidgetSessionId(value.widgetSessionId) ?? cleanWidgetSessionId(nestedOpenAi?.widgetSessionId);
}
function normalizeToolResponseMetadata(value) {
...
function toolInvocationBinding(metadata) {
...
function hostToolInvocationIdentity(hostContext, fallbackWidgetSessionId = null) {
  ...
  return {
    ...
    widgetSessionId: widgetSessionFromRecord(hostContext) ?? cleanWidgetSessionId(fallbackWidgetSessionId)
  };
}
function lateResultMessage(timeoutMs) {
  return `No valid result attached to this tool call within ${Math.round(timeoutMs / 1e3)} seconds. No action was taken. A valid late result can still appear here.`;
}
class ToolInvocationStore {
  ...
  initialLegacyHydrationConsumed = false;
  initialLegacyHydrationRecord = null;
  activeKey;
  constructor({
    clock = defaultClock(),
    ...
    this.clock = clock;
    this.timeoutMs = Number.isFinite(timeoutMs) && timeoutMs > 0 ? timeoutMs : TOOL_RESULT_ATTACH_TIMEOUT_MS;
  ...
  getSnapshot = () => this.activeRecord().snapshot;
  activateHostContext(hostContext, widgetSessionId = null) {
    ...
    if (current.trustedIdentity && compatibleIdentity(identity, current.snapshot.identity)) {
      const merged = {
        ...
        widgetSessionId: current.snapshot.identity.widgetSessionId ?? identity.widgetSessionId
      };
    ...
    if (!current.trustedIdentity && current.snapshot.status === "cancelled" && !current.snapshot.identity.toolName && !current.snapshot.identity.requestId && (!current.legacyWidgetSessionId || !identity.widgetSessionId || current.legacyWidgetSessionId === identity.widgetSessionId)) {
      current.trustedIdentity = true;
      this.rekeyActiveRecord(current, identity);
      this.emit();
      return true;
    }
    if (!current.trustedIdentity && this.initialLegacyHydrationRecord === current && !current.snapshot.identity.toolName && !current.snapshot.identity.requestId && (!current.legacyWidgetSessionId || !identity.widgetSessionId || current.legacyWidgetSessionId === identity.widgetSessionId)) {
      current.trustedIdentity = true;
    ...
    if (!current.trustedIdentity && current.acceptedUnboundLegacyHydration && current.snapshot.status === "ready" && !current.snapshot.identity.toolName && !current.snapshot.identity.requestId && (!current.legacyWidgetSessionId || !identity.widgetSessionId || current.legacyWidgetSessionId === identity.widgetSessionId)) {
      current.trustedIdentity = true;
      this.rekeyActiveRecord(current, {
        ...identity,
        widgetSessionId: identity.widgetSessionId ?? current.legacyWidgetSessionId
  ...
  acceptResult(result, source, {
    ...
    if (record.snapshot.status === "ready") {
      if (source === "mcp-apps" && !record.trustedIdentity && record.acceptedUnboundLegacyHydration && binding.kind === "valid") {
    ...
    if (record.trustedIdentity) {
      ...
      if (binding.kind === "valid") {
        ...
        const enriched = {
          ...
          widgetSessionId: expected.widgetSessionId ?? binding.identity.widgetSessionId
        };
  ...
  acceptMetadata(value) {
    const metadata = normalizeToolResponseMetadata(value);
    const binding = toolInvocationBinding(metadata);
    const record = this.activeRecord();
    if (record.snapshot.status === "ready" && !record.trustedIdentity && record.acceptedUnboundLegacyHydration && binding.kind === "valid") {
      return this.stageBoundMetadata(binding.identity);
    }
    return this.enrichReadyMetadata(record, metadata, binding);
  }
  acceptChatGptGlobals(globals, { allowUnboundLegacyHydration = false } = {}) {
    ...
    const binding = toolInvocationBinding(metadata);
    const globalSessionId = cleanWidgetSessionId(globals.widgetSessionId) ?? widgetSessionFromRecord(metadata);
    ...
    if (!current.trustedIdentity && binding.kind === "valid" && current.snapshot.status !== "ready" && current.snapshot.status !== "cancelled") {
      this.activate(binding.identity, true);
      current = this.activeRecord();
    }
    if (allowUnboundLegacyHydration && !this.initialLegacyHydrationConsumed && !this.initialLegacyHydrationRecord && binding.kind === "absent" && current.snapshot.status === "waiting" && current.snapshot.input === null && current.snapshot.output === null && current.snapshot.metadata === null && (current.trustedIdentity || !current.snapshot.identity.toolName && !current.snapshot.identity.requestId) && (!globalSessionId || !current.snapshot.identity.widgetSessionId || globalSessionId === current.snapshot.identity.widgetSessionId)) {
      this.initialLegacyHydrationRecord = current;
      current.legacyWidgetSessionId = globalSessionId;
    }
    const initialLegacyHydrationAvailable = !this.initialLegacyHydrationConsumed && this.initialLegacyHydrationRecord === current;
    const canAttachInitialUnboundLegacy = initialLegacyHydrationAvailable && binding.kind === "absent" && hasOutput && globals.toolOutput !== null && globals.toolOutput !== void 0 && current.snapshot.status !== "ready" && current.snapshot.status !== "cancelled" && current.snapshot.output === null && current.snapshot.metadata === null && (current.trustedIdentity || !current.snapshot.identity.toolName && !current.snapshot.identity.requestId) && (!globalSessionId || !current.snapshot.identity.widgetSessionId || globalSessionId === current.snapshot.identity.widgetSessionId) && (!globalSessionId || !current.legacyWidgetSessionId || globalSessionId === current.legacyWidgetSessionId);
    if (canAttachInitialUnboundLegacy) {
      this.initialLegacyHydrationConsumed = true;
    }
    let changed = false;
    if (hasInput && globals.toolInput !== null && globals.toolInput !== void 0) {
      const active = this.activeRecord();
      const bindingMatches = binding.kind === "valid" && sameIdentity(active.snapshot.identity, binding.identity);
      const canAcceptInitialUnboundLegacyInput = initialLegacyHydrationAvailable && binding.kind === "absent" && (allowUnboundLegacyHydration || canAttachInitialUnboundLegacy);
      if (!active.trustedIdentity || bindingMatches || canAcceptInitialUnboundLegacyInput) {
        changed = this.acceptInput(globals.toolInput, "chatgpt") || changed;
      }
    }
    if ((hasOutput || hasMetadata) && globals.toolOutput !== null && globals.toolOutput !== void 0) {
  ...
  cancel(reason) {
    ...
    this.update(record, {
      status: "cancelled",
      output: null,
      message: cleanReason ? `This tool call was cancelled: ${cleanReason}` : "This tool call was cancelled before a result arrived."
    });
    return true;
  }
  dispose() {
  ...
  activate(identity, trustedIdentity) {
    const key = invocationKey(identity);
    if (key === this.activeKey) {
      const record2 = this.activeRecord();
      record2.trustedIdentity = record2.trustedIdentity || trustedIdentity;
      return false;
    }
    this.activeKey = key;
  ...
  compatibleInactiveRecords(identity, current) {
    const exact = this.records.get(invocationKey(identity));
    if (exact && exact !== current && exact.trustedIdentity) return [exact];
    return [...this.records.values()].filter((record) => record !== current && record.trustedIdentity && compatibleIdentity(record.snapshot.identity, identity));
  }
  activateExistingRecord(record, identity) {
  ...
  getOrCreateBoundRecord(identity) {
    const key = invocationKey(identity);
  ...
  stageBoundMetadata(identity) {
    const { created } = this.getOrCreateBoundRecord(identity);
    return created;
  }
  stageBoundResult(result, metadata, identity) {
  ...
  createRecord(identity, trustedIdentity) {
  ...
  scheduleTimeout(record) {
    ...
    const handle = record.timeoutHandle;
    handle?.unref?.();
  }
  rekeyActiveRecord(record, identity) {
  ...
  update(record, patch) {
    record.snapshot = {
      ...record.snapshot,
      ...patch,
      identity: patch.identity ? { ...patch.identity } : record.snapshot.identity,
      updatedAt: this.clock.now()
    };
    if (record.snapshot.key === this.activeKey) this.emit();
  }
  enrichReadyMetadata(record, metadata, binding) {
    if (record.snapshot.status !== "ready" || record.snapshot.metadata !== null || !metadata || binding.kind !== "valid") {
      return false;
    }
    ...
    const enrichedIdentity = {
      toolName: expected.toolName ?? binding.identity.toolName,
      requestId: expected.requestId ?? binding.identity.requestId,
      widgetSessionId: expected.widgetSessionId ?? binding.identity.widgetSessionId ?? record.legacyWidgetSessionId
    };
    record.trustedIdentity = true;
...
const ZERO_SAFE_AREA = {
  top: 0,
  right: 0,
  bottom: 0,
  left: 0
};
function createDefaultHostContext() {
  return {
    theme: "dark",
    displayMode: "inline",
...
function normalizeMcpToolInput(params) {
  if (!params || typeof params !== "object" || Array.isArray(params)) return params;
  const record = params;
  return record.arguments ?? params;
}
function normalizeMcpHostContext(value, previous = createDefaultHostContext()) {
  const context = value ?? {};
  const containerDimensions = context.containerDimensions && typeof context.containerDimensions === "object" ? context.containerDimensions : previous.containerDimensions;
  const deviceCapabilities = context.deviceCapabilities && typeof context.deviceCapabilities === "object" ? context.deviceCapabilities : previous.deviceCapabilities;
  const safeAreaInsets = context.safeAreaInsets && typeof context.safeAreaInsets === "object" ? {
    ...previous.safeAreaInsets,
    ...context.safeAreaInsets
  } : previous.safeAreaInsets;
  const styles = context.styles && typeof context.styles === "object" ? context.styles : previous.styles;
  const availableDisplayModes = Array.isArray(context.availableDisplayModes) ? context.availableDisplayModes.filter(
    (mode) => mode === "inline" || mode === "fullscreen" || mode === "pip"
  ) : previous.availableDisplayModes;
  const rawToolInfo = context.toolInfo;
  const toolInfo = rawToolInfo && typeof rawToolInfo === "object" && !Array.isArray(rawToolInfo) && "tool" in rawToolInfo && rawToolInfo.tool && typeof rawToolInfo.tool === "object" && !Array.isArray(rawToolInfo.tool) && "name" in rawToolInfo.tool && typeof rawToolInfo.tool.name === "string" ? rawToolInfo : previous.toolInfo;
  const widgetSessionId = typeof context["openai/widgetSessionId"] === "string" ? context["openai/widgetSessionId"] : typeof context.widgetSessionId === "string" ? context.widgetSessionId : previous.widgetSessionId;
  return {
    ...previous,
    ...context.theme === "light" || context.theme === "dark" ? { theme: context.theme } : {},
    ...context.displayMode === "inline" || context.displayMode === "fullscreen" || context.displayMode === "pip" ? { displayMode: context.displayMode } : {},
    availableDisplayModes: availableDisplayModes.length ? availableDisplayModes : previous.availableDisplayModes,
    containerDimensions,
    ...
    platform: context.platform === "web" || context.platform === "desktop" || context.platform === "mobile" ? context.platform : previous.platform,
    deviceCapabilities,
    safeAreaInsets,
    styles,
    toolInfo,
...
function supportsTextModality(value) {
  return Boolean(
    value && typeof value === "object" && !Array.isArray(value) && "text" in value
  );
}
function normalizeMcpCapabilities(capabilities, hostContext) {
...
let _hostContext = {};
let _hostCapabilities = {};
let _messageListenerAttached = false;
function getDeclaredDisplayModes() {
  if (typeof document === "undefined") return ["inline"];
  const content = document.querySelector('meta[name="dexter-app-display-modes"]')?.content;
  const modes = content?.split(/\s+/).filter((mode) => mode === "inline" || mode === "fullscreen" || mode === "pip");
...
function dispatchNotification(method, params) {
  const handlers2 = _notificationHandlers.get(method);
  if (!handlers2) return;
...
function handleMessage(event) {
  ...
  if ("method" in data && !("id" in data)) {
    if (data.method === "ui/notifications/host-context-changed" && data.params && typeof data.params === "object" && !Array.isArray(data.params)) {
      _hostContext = {
        ..._hostContext,
        ...data.params
      };
...
function onNotification(method, handler) {
  let handlers2 = _notificationHandlers.get(method);
  if (!handlers2) {
    handlers2 = /* @__PURE__ */ new Set();
    _notificationHandlers.set(method, handlers2);
  }
...
async function openLink(href) {
  let result;
  try {
    result = await sendRequest("ui/open-link", { url: href });
  } catch (error2) {
    const opened = window.open(href, "_blank", "noopener,noreferrer");
    if (!opened) throw error2;
    return;
  }
  if (result?.isError) {
...
async function openLinkProbe(href) {
  try {
    ...
    return result?.isError ? { ok: false, error: "The host declined to open this link" } : { ok: true, response };
  } catch (error2) {
    return {
      ok: false,
...
async function updateModelContext(context) {
  await sendRequest("ui/update-model-context", {
    ...context.text ? { content: [{ type: "text", text: context.text }] } : {},
...
let _chatGptDisplayModeOverride = null;
const _chatGptAdapterListeners = /* @__PURE__ */ new Set();
let _mcpTheme = "dark";
let _mcpHostContext = DEFAULT_HOST_CONTEXT;
let _mcpHostCapabilities = DEFAULT_HOST_CAPABILITIES;
let _mcpInitDone = false;
const _mcpListeners = /* @__PURE__ */ new Set();
const _adaptiveSnapshotCache = /* @__PURE__ */ new Map();
let _chatGptInvocationHydrationState = null;
function invocationHydrationState(globals) {
  return {
...
function invocationClock() {
  if (typeof window === "undefined") return void 0;
  const candidate = window.__dexterToolInvocationClock;
  if (!candidate || typeof candidate !== "object" || Array.isArray(candidate)) return void 0;
  const value = candidate;
  if (typeof value.now !== "function" || typeof value.setTimeout !== "function" || typeof value.clearTimeout !== "function") return void 0;
  return {
    now: value.now,
...
function stableSnapshot(key, value) {
  try {
    const serialized = JSON.stringify(value);
    const cached = _adaptiveSnapshotCache.get(key);
    if (cached?.serialized === serialized) return cached.value;
    _adaptiveSnapshotCache.set(key, { serialized, value });
  } catch {
  }
  return value;
}
function getChatGptHostContext(fallback = DEFAULT_HOST_CONTEXT) {
  const openai = typeof window !== "undefined" ? window.openai : void 0;
  const displayMode = _chatGptDisplayModeOverride ?? openai?.displayMode ?? fallback.displayMode;
  const safeArea = openai?.safeArea?.insets ?? fallback.safeAreaInsets;
  const maxHeight = typeof openai?.maxHeight === "number" && Number.isFinite(openai.maxHeight) ? openai.maxHeight : fallback.containerDimensions?.maxHeight;
  const platform = openai?.userAgent?.device?.type === "mobile" ? "mobile" : openai?.userAgent?.device?.type === "desktop" ? "desktop" : fallback.platform;
  return stableSnapshot("chatgpt-host-context", {
    ...fallback,
    theme: openai?.theme ?? fallback.theme,
    displayMode,
    availableDisplayModes: typeof openai?.requestDisplayMode === "function" ? Array.from(/* @__PURE__ */ new Set([
    ...
    platform,
    deviceCapabilities: openai?.userAgent?.capabilities ?? fallback.deviceCapabilities,
...
function chatGptInvocationGlobals(changedGlobals) {
  if (typeof window === "undefined" || !window.openai) return null;
  const changed = changedGlobals ?? {};
  const has = (key) => Object.prototype.hasOwnProperty.call(changed, key);
  const includeAll = changedGlobals === void 0;
  const invocationChanged = includeAll || has("toolInput") || has("toolOutput") || has("toolResponseMetadata") || has("widgetSessionId");
  if (!invocationChanged) return null;
  const globals = window.openai;
  return {
    ...includeAll || has("toolInput") ? { toolInput: has("toolInput") ? changed.toolInput : globals.toolInput } : {},
    ...
    ...includeAll || has("toolOutput") ? { toolOutput: has("toolOutput") ? changed.toolOutput : globals.toolOutput } : {},
    ...includeAll || has("toolResponseMetadata") || has("toolOutput") ? {
      toolResponseMetadata: has("toolResponseMetadata") ? changed.toolResponseMetadata : globals.toolResponseMetadata
    } : {},
    widgetSessionId: has("widgetSessionId") ? changed.widgetSessionId : globals.widgetSessionId
  };
}
function hydrateChatGptInvocation() {
  ...
  if (previous && Object.is(previous.toolInput, next.toolInput) && Object.is(previous.toolOutput, next.toolOutput) && Object.is(previous.toolResponseMetadata, next.toolResponseMetadata) && Object.is(previous.widgetSessionId, next.widgetSessionId)) return;
...
function initMcpAppsOnce() {
  ...
  onNotification("ui/notifications/tool-cancelled", (params) => {
    const reason = params && typeof params === "object" && !Array.isArray(params) ? params.reason : null;
...
function subscribeChatGPTGlobals(onChange) {
  if (typeof window === "undefined") return () => {
  };
...
function subscribeAdaptive(onChange) {
...
function chatGptGlobal(key) {
  if (typeof window === "undefined" || !window.openai) return void 0;
  const value = window.openai[key];
  return value === void 0 ? void 0 : value;
}
function getAdaptiveHostContext() {
  if (typeof window === "undefined" || !window.openai) return _mcpHostContext;
  return getChatGptHostContext(_mcpHostContext);
}
function getAdaptiveCapabilities() {
  ...
  return stableSnapshot("adaptive-capabilities", {
    ...
    requestDisplayMode: chatGpt.requestDisplayMode || _mcpHostCapabilities.requestDisplayMode,
    updateModelContext: chatGpt.updateModelContext || _mcpHostCapabilities.updateModelContext,
    sendFollowUpMessage: chatGpt.sendFollowUpMessage || _mcpHostCapabilities.sendFollowUpMessage,
...
function useToolOutput() {
  return reactExports.useSyncExternalStore(
    ...
    () => {
      hydrateChatGptInvocation();
      return _invocations.getSnapshot().output ?? null;
    },
    () => null
  );
}
function useToolResponseMetadata() {
  return reactExports.useSyncExternalStore(
    ...
    () => {
      hydrateChatGptInvocation();
      return _invocations.getSnapshot().metadata ?? null;
    },
    () => null
  );
}
function useToolInput() {
  return reactExports.useSyncExternalStore(
    ...
    () => {
      hydrateChatGptInvocation();
      return _invocations.getSnapshot().input ?? null;
    },
    () => null
  );
}
function useToolInvocationLifecycle() {
  return reactExports.useSyncExternalStore(
    subscribeAdaptive,
    () => {
      hydrateChatGptInvocation();
...
function useAdaptiveTheme() {
  return reactExports.useSyncExternalStore(
    subscribeAdaptive,
    () => chatGptGlobal("theme") ?? _mcpTheme,
    () => "dark"
  );
}
function useAdaptiveHostContext() {
  return reactExports.useSyncExternalStore(
    subscribeAdaptive,
...
function useAdaptiveHostCapabilities() {
  return reactExports.useSyncExternalStore(
    subscribeAdaptive,
...
function useAdaptiveUpdateModelContext() {
  const capabilities = useAdaptiveHostCapabilities();
  const update = reactExports.useCallback(async (value) => {
    if (isInitialized() && _mcpHostCapabilities.updateModelContext) {
      await updateModelContext(value);
      return;
    }
    if (typeof window !== "undefined" && window.openai?.updateModelContext) {
      const content = value.text ? [{ type: "text", text: value.text }] : void 0;
...
function useAdaptiveSendFollowUp() {
  const capabilities = useAdaptiveHostCapabilities();
  const send = reactExports.useCallback(async (prompt) => {
    if (isInitialized() && _mcpHostCapabilities.sendFollowUpMessage) {
...
function useAdaptiveCallToolFn() {
  return reactExports.useCallback(async (name, args) => {
    if (isInitialized() && _mcpHostCapabilities.callTool) {
...
function useAdaptiveOpenExternal() {
  return reactExports.useCallback((href) => {
    if (isInitialized() && _mcpHostCapabilities.openExternal) {
...
export {
  React as R,
  useAdaptiveTheme as a,
  useAdaptiveMaxHeight as b,
  useAdaptiveDisplayMode as c,
  useAdaptiveHostContext as d,
  useAdaptiveHostCapabilities as e,
  useAdaptiveRequestDisplayMode as f,
  useToolResponseMetadata as g,
  useAdaptiveOpenExternal as h,
  openLinkProbe as i,
  jsxRuntimeExports as j,
  requireReact as k,
  getDefaultExportFromCjs as l,
  useAdaptiveCallToolFn as m,
  useAdaptiveSendFollowUp as n,
  openLink as o,
  useAdaptiveUpdateModelContext as p,
  useToolInvocationLifecycle as q,
  reactExports as r,
  useToolInput as s,
  addWidgetBreadcrumb as t,
  useToolOutput as u,
  captureWidgetException as v,
  commonjsGlobal as w
};
