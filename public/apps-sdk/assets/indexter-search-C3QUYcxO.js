import { a as useAdaptiveTheme, j as jsxRuntimeExports, r as reactExports, m as addWidgetBreadcrumb, n as captureWidgetException, u as useToolOutput, p as useToolInput, d as useAdaptiveHostContext, e as useAdaptiveHostCapabilities, b as useAdaptiveMaxHeight, c as useAdaptiveDisplayMode, f as useAdaptiveRequestDisplayMode, q as useAdaptiveSendFollowUp } from "./adapter-DvI1aAxR.js";
/* empty css             */
import { c as clientExports } from "./client-BiBV5Ase.js";
import { p as providerImageSources, f as formatListedPrice, a as formatAssetLabel, h as hostLabel } from "./utils-DUJSm44K.js";
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "dx-search-drawer__footer", children: [
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        "button",
        {
          ...
          children: action.disabled ? action.label : !onUseService ? "Unavailable in this host" : checking ? action.kind === "provide_details" ? "Opening chat…" : "Opening terms in chat…" : action.label
        }
      )
    ] }),
...
function SearchDecisionBrief({
  ...
  const actionLabel = resourceAction.disabled ? resourceAction.label : unavailableInHost ? "Unavailable in this host" : isChecking ? resourceAction.kind === "provide_details" ? "Opening chat…" : "Opening terms…" : detailsSent ? "Opened in chat" : relevantCheckState.status === "error" ? "Try again" : resourceAction.label;
  const actionDisabled = isChecking || interactionLocked || detailsSent || resourceAction.disabled || !canPerformAction;
  const actionNote = resourceAction.disabled ? resourceAction.helperText : unavailableInHost ? resourceAction.kind === "provide_details" ? "This host can't continue the request in chat." : "This host can't open the current-terms check in chat." : relevantCheckState.status === "error" ? relevantCheckState.message : relevantCheckState.status === "checking" ? relevantCheckState.message || "Opening the terms check in chat…" : relevantCheckState.status === "details_sent" ? relevantCheckState.message || "Continue in chat to provide the missing request details." : resourceAction.helperText;
  if (compact) {
...
const SEARCH_WIDGET_BUILD = "2026-09-03.3";
