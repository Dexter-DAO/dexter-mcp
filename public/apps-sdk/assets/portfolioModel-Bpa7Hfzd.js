...
const MAX_CAPABILITY_REASON_LENGTH = 2048;
...
function parseCapability(value) {
  ...
  if (value.available ? value.reason !== null : !isNonEmptyString(value.reason) || value.reason.length > MAX_CAPABILITY_REASON_LENGTH) return null;
  return {
    action: value.action,
...
function parseHolding(value) {
  ...
  return {
    ...
    issuer: value.issuer === null ? null : boundedDisplayText(value.issuer, 96),
...
function compactDecimal(value, prefix = "") {
  const negative = value.startsWith("-");
  const absolute = negative ? value.slice(1) : value;
  ...
  if (group < suffixes.length) {
    ...
    const decimals2 = `${whole.slice(leadingLength)}${fraction}`.slice(0, 2).replace(/0+$/, "");
    return `${prefix}${sign}${leading}${decimals2 ? `.${decimals2}` : ""}${suffixes[group]}`;
  }
