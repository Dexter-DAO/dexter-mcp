...
The wallet widget uses its existing private refresh token to read `/widget/wallet/activity`. Opening Activity refreshes the newest page. Refresh and Load older activity preserve the shared cursor and show read failures. Rows show service or asset artwork, the action, service descriptions, and any pending or failed status. Service names open their Indexter page; amounts and timestamps open the transaction. Expand a row for exact trade quantities or additional receipt facts. A secondary transaction remains available through a chain icon with a descriptive tooltip. Atomic amounts and transaction display quantities remain strings through display, including subcent payments and large token quantities.
